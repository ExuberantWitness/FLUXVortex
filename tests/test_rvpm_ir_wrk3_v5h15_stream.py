...
import fluxvortex.rvpm_ir_wrk3_v5h15_stream as stream_module
...
def test_result_tree_is_exact_readonly_deterministic_and_validator_fails_closed() -> (
    ...
    # V5H15 binds delta_time exactly to the per-record sub-step grid, so a
...
def test_graded_birth_window_grid_keeps_minimum_order(jacobian: np.ndarray) -> None:
    """V5H15 analytic oracle: the graded grid stays >= MINIMUM_ORDER and is
    ...
    # Preregistered V5H15 guard: temporal order >= 2 on the graded grid
