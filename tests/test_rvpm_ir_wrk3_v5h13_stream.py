...
import fluxvortex.rvpm_ir_wrk3_v5h13_stream as stream_module
...
def test_result_tree_is_exact_readonly_deterministic_and_validator_fails_closed() -> (
    ...
    # V5H13 binds delta_time exactly to the per-record sub-step grid, so a
    # one-ulp tamper is now rejected by the tree validator itself (stronger
    # than the registry-only rejection the uniform-only lineage relied on).
    with pytest.raises(ValueError, match="sum exactly to delta_time"):
        stream_module._validate_result_tree(modified)
    with pytest.raises(ValueError, match="trusted macro|registration"):
        stream_module._register_result(modified)
    assert _registry_snapshot() == before_registry
    with pytest.raises(
        ValueError, match="sum exactly to delta_time|live issued|exact live"
    ):
...
def test_graded_birth_window_schedule_integrates_verbatim_and_binds_records() -> None:
    import math

    initial = _initial_state()
    graded = ir_wrk3_stream_macro(
        initial,
        0.0175,
        _affine_field(JACOBIANS[2]),
        transport_substeps=7,
        tracer_positions=_tracers(),
        tracer_field_evaluator=_affine_tracer(JACOBIANS[2]),
        parent_token=PARENT_TOKEN,
        substep_delta_times=(0.0175 / 16.0,) * 4 + (0.0175 / 4.0,) * 3,
    )
    assert graded.transport_substeps == 7
    assert len(graded.stages) == 3 * 7
    per_substep = [graded.stages[3 * j].substep_delta_time for j in range(7)]
    assert per_substep == [0.0175 / 16.0] * 4 + [0.0175 / 4.0] * 3
    assert all(
        graded.stages[3 * j + i].substep_delta_time == per_substep[j]
        for j in range(7)
        for i in range(3)
    )
    assert math.fsum(per_substep) == graded.delta_time
    assert validate_ir_wrk3_stream_result(graded) is graded

    with pytest.raises(ValueError, match="graded substep count"):
        ir_wrk3_stream_macro(
            initial,
            0.0175,
            _affine_field(JACOBIANS[2]),
            transport_substeps=7,
            substep_delta_times=(0.0175 / 16.0,) * 4 + (0.0175 * 3.0 / 8.0,) * 2,
        )
    with pytest.raises(ValueError, match="sum to delta_time"):
        ir_wrk3_stream_macro(
            initial,
            0.0175,
            _affine_field(JACOBIANS[2]),
            transport_substeps=6,
            substep_delta_times=(0.0175 / 16.0,) * 4 + (0.0175 / 4.0,) * 2,
        )
    with pytest.raises(ValueError, match="graded substep count"):
        ir_wrk3_stream_macro(
            initial,
    ...
    with pytest.raises(ValueError, match="positive finite float"):
        ir_wrk3_stream_macro(
            ...
            _affine_field(JACOBIANS[2]),
            transport_substeps=2,
            substep_delta_times=(0.0125, -0.0),
        )
    wrong_slot = replace(
        graded.stages[4],
        substep_delta_time=float(
            np.nextafter(graded.stages[4].substep_delta_time, np.inf)
        ),
        record_sha256="",
        chain_sha256="",
    )
    with pytest.raises(ValueError, match="grid slot|sum exactly"):
        stream_module._validate_result_tree(
            replace(graded, stages=(*graded.stages[:4], wrong_slot, *graded.stages[5:]))
        )


@pytest.mark.parametrize("jacobian", JACOBIANS)
def test_graded_birth_window_grid_keeps_minimum_order(jacobian: np.ndarray) -> None:
    """V5H13 analytic oracle: the graded grid stays >= MINIMUM_ORDER and is
    never less accurate than the uniform grid at the same nominal N."""

    from forward_flight_benchmarks.fluxv_v5h13_baik_coupling import (
        graded_substep_delta_times,
    )

    initial = _initial_state()
    expected = _affine_exact(initial, jacobian, HORIZON_S)
    graded_errors: list[float] = []
    uniform_errors: list[float] = []
    graded_levels = tuple(n for n in ORDER_LEVELS if n >= 5)
    for substeps in graded_levels:
        schedule = graded_substep_delta_times(HORIZON_S, substeps)
        graded = ir_wrk3_stream_macro(
            initial,
            math.fsum(schedule),
            _affine_field(jacobian),
            transport_substeps=len(schedule),
            parent_token=PARENT_TOKEN,
            substep_delta_times=schedule,
        )
        validate_ir_wrk3_stream_result(graded)
        uniform = ir_wrk3_stream_macro(
            initial,
            HORIZON_S,
            _affine_field(jacobian),
            transport_substeps=substeps,
        ...
        graded_errors.append(
            _relative_l2(graded.final_state.positions, expected.positions)
        )
        uniform_errors.append(
            _relative_l2(uniform.final_state.positions, expected.positions)
        )
    orders = tuple(
        log2(coarse / fine)
        for coarse, fine in zip(graded_errors[:-1], graded_errors[1:])
    )
    # Preregistered V5H13 guard: temporal order >= 2 on the graded grid
    # (the uniform-only 2.8 threshold does not govern the mixed grid, whose
    # apparent order rises toward 3 pre-asymptotically through fine/coarse
    # error cancellation; graded is strictly more accurate than uniform at
    # every level in practice).
    assert graded_errors[-1] <= 1.0e-7, graded_errors
    assert all(order >= 2.0 for order in orders[-2:]), (
        graded_errors,
        orders,
    )
    assert all(g <= u * 1.5 + 1.0e-15 for g, u in zip(graded_errors, uniform_errors)), (
        graded_errors,
        uniform_errors,
    )
