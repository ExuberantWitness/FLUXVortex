...
### 5.0 P1 双计 bug 修复(2026-07-05,commit 2b39413)+ 残留分离流过预测

### 5.0b P2a 柔性翼 FSI 发散诊断(2026-07-05,commit f03fc17)—— 结构模型根因
实验翼是**三材料复合膜翼**(主碳梁管@30%弦 + 辅碳梁管@70%弦 + 7 航空层板翼肋 + 翼根铝合金肋 + 薄膜),增强刚度但仍柔性(实测弦向 k=166.9 N/m)。当前 `_v2_flex_robo.py` 的 ANCF 壳 FSI **在所有材料配置下发散**:
- 三材料壳(set_distribution:碳梁条+层板肋条+膜):bend 1e27-1e38,finite=False(立即)。
- 均匀正交异性 Ey=50-200GPa + added_mass + 阻尼 0.3-0.6:bend 5e3-1e10,finite=False。

**根因(深层)**:① **ANCF 壳无预张力**(无几何刚度 Kσ)→ 膜是软塌板,不是绷在梁框上的鼓皮;Ex_mem 标到 0.3MPa 后膜几乎不承力,梁条之间无剪切完整性 → 折叠。② **薄壳条带不能表示梁弯曲** EI(壳条 EI=E·w·h³/12 即使 E=150GPa,h=1.2mm,也比真实碳管 EI 小,且梁-膜连接无剪切传递)。③ fluxvortex 只有 1D BeamFE,**无 3D 梁、无预张力膜**。

**P2 正解(用户定:ANCF+3D梁组合)**:3D 梁框架(翼梁/翼肋,承担弯曲 EI+扭转 GJ)+ 预张力膜壳(面内张力→几何刚度 Kσ,鼓皮行为)+ 共享节点耦合。三者需新建(3D ANCF 梁/3D frame 元素、预张力壳、壳-梁耦合)。这是 P2 的核心结构 FE 工程(多日)。
**暂态**:_v2_flex_robo ANCF 壳路径不可用;H16 刚性翼仍是产线最优(推力 MAE 2.11)。

### 5.0c P2-S0 预张力去风险(2026-07-05,commit d3dc3fa)—— 预张力单独不够
加 `ancf_shell.set_pre_tension(N0)`(种子化 2nd-PK 预应力,进 Q_mem 力 + K_mem 几何刚度 Kσ)。去风险扫:N0=0/100/500/2000/10000 N/m 给**完全相同**的发散数(1e77,finite=False)→ **预张力零效果**。
- 隔离验证:FlapUVLMProvider 在刚性静止几何给**合理升力 4.37N**(finite)→ 气动非垃圾。
- **真根因**:`calibrate_Ex` 把均匀膜 Ex 压到 2.2MPa(匹配实测 k=166.9 时把**肋的刚度抹进膜**)→ 弯曲刚度 Dk=Ex·h³/12 极小 → 壳**面外折叠**。预张力只补**面内** Kσ,补不了**面外弯曲**发散。
- **结论**:预张力单独不够;弯曲发散需**梁框架**(翼梁/肋承担弯曲 EI)。确认 P2-S1(3D 梁)+ S2(张力场膜)必要。且均匀壳校准本身误模(肋刚度抹进膜)——标准 FEM 梁+膜分离建模才对。
H19 的 `prof_drag`(Hoerner)叠加到未封顶 bound Bernoulli 后倾分量 → 深扭转双计过阻 −4.70N。**修法 `kirch_blend`**:附着 Bernoulli(环量+加质量)与平板 CN 法向压力(q·Cd_max·sin²α·sign)按(滞后)分离分数 (1−fsep) 混成**单一矢量**——压力阻力从同一矢量涌现(升力塌缩+阻力),不叠加。`prof_drag` 在 blend 开时门控关。
- **结果(H20 = H16 impulse + kirch_blend + visc + fsep_lag,全 122 工况)**:均值 −1.36(H19 −4.70→ 改善)、MAE 3.69、符号 71%。Fig16 tw22.5/tw45 推力 dMean −5.36/−9.91 → −1.18/−7.05(过冲减小)。K0 逐位不变。
- **诚实结论**:**双计结构性消除**(单矢量,物理干净),**但 H20 MAE 3.69 > H16 2.11**——平板 CN(`cd_form=1.98`)本身过预测分离法向力,即使无双计也比 H16(纯涡冲量,无分离闭合)差。证实:双计是 bug(已修),深扭转过预测的根是**分离流法向力模型过粗 + 动态失速时间尺度**(GK τ*=4.5 让 tw0 分离延迟过多 → tw0 回退到 H16 附着水平 +2.69)。
- **H16 仍为产线最优**(MAE 2.11)。H20 是 bug-free 中间态;精度提升需 P2(FSI 改变形变/有效几何)+ 合适分离流模型。
