...
## 附:D2 判别线判决(2026-07-16,物理 d_para=0.5 + R2 门,T3b step-1)

```
gapT      f1.4    f2.0    f2.6     线内结构
U6(Re1.1e5) +3.45  +3.52  +3.10    平(f 无关)
U8(Re1.5e5) +2.22  +2.19  +1.96    平
U10(Re1.9e5)+0.54  +0.63  +0.38    平(近闭合)
升力:U6 ±0.25 近完美;U8/U10 与已知 L2 结构一致
```

**判定**:①线内 f 平坦 → 轨三全部 k 依赖候选(Garrick c1/c2、Moored-Quinn)被
处决——R2+plateau 已把三速度的 f 斜率全部做对;②T3b 残差 = 纯 U 函数(f 无关、
随 U 线性降、U≈10.5 归零);③量级(U6 ΔCD~0.37)超翼型级机理两个量级 →
系统级来源候选:机构尾流-机翼干扰(随 U 减弱?)、论文数据处理的 U 依赖基线
(各 U 线是否独立坐零?)、低 Re 平均流分离(Re 1.1e5→1.9e5 恰跨低 Re 阻力桶)。
判别探针二(运行中):tw0 的 U6/U10 线——偏移在 tw0 复现→静态/基线来源;
不现→feathering 相关。**d_para=0.5×(U/8)² 物理值确立为生产值;T3b 独立立案,
禁以常数吸收。**

## 附:T3b 终裁(2026-07-16)

三方证据闭合:①D2 线内 f 平坦(杀全部 k 候选);②Fig15 三速系数重合(杀静态
气动来源);③模型 tw0 U 结构平滑(杀模型 U 病态)。分辨 U8 与 U6/U10:
- **U8 份额 +2.1N 是真的**(U8 线在论文内跨 Fig17/18/19 交叉自洽)——扑动激活、
  f 幅值无关、随 U 下降的阻力缺失,机理未明(候选:翼根间隙/机构区扑动激活分离;
  其 U 趋势反常,无现成文献律);
- **U6/U10 绝对零位不可验证**(论文内无交叉点;Fig17/18 共享点 0.73N 分歧证明
  会话级基线偏移存在)→ 登记为实测基线不确定带(≤±1.5N 量级叠加在真实份额上)。
**处置:d_para=0.5×(U/8)²(物理,滑翔极曲线钉死)为生产值;T3b 作为可见开放案
入图注;禁止任何常数吸收(两错互抵红线)。相对结构(f 斜率/扭转趋势/攻角分层)
为可信对标量,绝对推力带 T3b 不确定度。**

## 附:T3b-trend 重定性(2026-07-16,用户裁定)

**用户判定采纳:平移(d_para 口径)不重要,dT/dU 趋势反转才是致命伤。**
tw22.5/f1.4:实测 T 随 U 升(+0.50 N/(m/s):−3.17→−2.29→−1.16);模型随 U 降
(−0.23 N/(m/s)),v2/v3 两口径同病——旧 U² 补偿律只是遮掩(代价=U10 线 MAE 3.28)。
**T3b 案主题从"缺常数阻力"改判为"dT/dU 符号错误"(结构性),平移项一并附属。**
机理嫌疑(与 a0_crit 有效参数案合流):吸力帽 A0≤0.27 在低 U(高 k)过供推力
(U6 +3.4 过剩)、高 U 自然回落——三点扫旁证:无单一常数同时伺候三速度 →
帽物理需换随状态判据(Tier-1 BL,优先级升至队首)。通道归因探针运行中
(U×f 分解 dT/dU 的通道来源)。
