...
- **✖ CLOSED(2026-07-19,用户裁定:精度不可用,彻底关闭)**:唯一有效 3D 测点
  升力 1.1N vs 实测 7.8N(−86%),力通道缺件(gap_s3_force.md 归因:折入压塌+
  无补偿通道)未修即结案。不再复活。已验证资产留档备查:S1 输运门、S2 二维门
  (ldvm_fourier.py,发表基准全落靶)、S3b 稳定七修链;part_mode='rvpm' 代码路径
  与 rvpm3d.py 保留但不维护(CLOSED 注记)。生产口径 = v4(从未被替换;退出验证
  U6 红线工况对 v4 缓存 Δ=+0.016/+0.007 噪声带内)。
...
- **S2 过门(2026-07-18)**:2D 参考测试台 = **ldvm_fourier.py LDVM2D**(UNSflow
  `ldvm` 逐行保真移植:Fourier 束缚层 naterm=35/ndiv=70、每步一粒/1/3 放置/2×2 闭式
  (Faure 形)/Vatistas rc=0.02c/环量冻结/pre-LEV 率项力公式)。案定义与力公式经
  作者博士论文+JFS2015+UNSflow 三方核实(research_rvpm_s2cases.md,含 Table 4.1
  印刷错位勘误:各案 crit=0.18(SD7003@3e4)/0.11(平板@1e3);**旧摘要"0.18/0.14"
  归属有误,0.14 属 SD7003@1e5**)。
  - G3a(移植保真门,全过+加密无关 ±1.5%):A(5B 平板45°K0.4)尖峰 6.35@1.69/
    hold 均 2.63∈[1.5,3];B(Case1 SD7003 25°K0.11)峰 2.84@2.76 正中 2.7-2.8@2.5-3.0/
    回程谷 −0.58;C(5A 平板90°K0.2)首峰 2.29(靶2.4)/主峰 4.22@2.46(靶4.0@2.5)/
    Cd 峰 4.2(靶4.2)。对照:集中涡逐点解在 n=140 加密下爆至 164——Fourier 截断
    低通是承重结构(历史"加密不收敛"谱系再添一案)。
  - G3b(物理门,对 Visbal 系 LES):相位 102°∈[100,120]✓、峰处 α_eff 21.7°✓、
    CL_min −0.13~−0.15✓;**CL 峰 +2.89 vs 2.3-2.4(+20%)对 crit∈{0.14,0.18} 稳健
    = 无粘 LDVM 系统性偏差,CD 峰 0.28 低 20%**——正是 rVPM(SFS/粘性)S4-S5 靶点,
    注记不吸收。图 rvpm_s2_gates.png。
  - 移植期修正三错(尾迹项符号约定混用/plunge ḣ 符号/率项 pre-LEV 语义)——
    率项语义是"中段升力缺 1-1.5"的全因;flap_ldvm.py 保留为 legacy 条带闭合
    (其 h 下正约定自洽勿动),新增 S2 选项不改缺省(阶梯逐位回归过)。
