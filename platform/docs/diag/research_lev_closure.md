...
## 附:R2 实现与验收(2026-07-16)

`les_sep='plateau_fn'` 已实现(_v2_robo.py,fn_state 逐条带形成数累积,T*=4.0 文献值):
9 工况门电池 vs plateau 基线——惰性门全过(tw≤30/巡航/f线/U6,|Δ|≤0.13);tw45
+0.99→+0.66(方向 ✓,回收 31%<50% 期望,软点);tw30 绝对误差 0.06→0.14(软点);
**aoa15/f2.6 +0.46→−0.16(红利)**;升力逐位不动。**验收:通过,收入生产候选。**
后续:a0_crit 三点扫(帽降会加深门的作用);tw45 剩余 0.66 与 R3(A0* 迟滞)候补。
