...
**时间**：2026-07-28T21:45:38+08:00  
**artifact**：`actual_wake_reachable_pressure_authorization_schema_v23_preregistration_20260728_214538`  
...
- 21:19:24 第六版 successor MD SHA
  `5c0aad0637aeda05db0884ffaf73154e4b3d5c7c54415aed801a1b59f5d10e2c`、
  JSON SHA
  `bd743d6e251cb76aaf6b914aff9d2c7c66936a0cbbe8f624c6e7c24f3fab45f6`；
- 21:32:19 两路只读第七次复审与 sidecar cross-check
  `FAIL / 3 remaining blocker groups` MD SHA
  `c2999774163ec7a5379bad4755e8fed06fec9baab3d024a171286f8efc4f0980`、
  JSON SHA
  `3c8ed31015696dcb819e06a4ab3f385bb516080faadbe4d63439a219410ad5cf`。
- 21:34:09 第七版 successor MD SHA
  `d6da25243e6d821e5bb6e20b384d1ee2fc8bea9f09638bd59dbf05f6ca802cf9`、
  JSON SHA
  `cae6bddddce673d7b6f25ab8f32528d28e17c997d7380e3b10a2c39e017da3ca`；
- 21:44:56 两路只读第八次复审与 sidecar cross-check
  `FAIL / 2 remaining blocker groups` MD SHA
  `a8d5d0a4c4cfd23ae3637399d83d8cc3673f2527fed48fcf6a68991fcc81847a`、
  JSON SHA
  `0fa772cff78d1f33bbce7abf4e77312fb9ee2a09a0ab01b548d4536e86175423`。
...
filesystem authority。profile不是 JSON artifact。所有 directory authority
只能由以下 opaque typed lease 表达：

```text
_DirectoryLeaseV1 exact fields:
  absolute_path: absolute Path
  fd: nonnegative integer, not bool
  st_dev: nonnegative integer, not bool
  st_ino: positive integer, not bool
  owner_created: exact bool
```

lease 的 `fd` 必须仍 open，`FD_CLOEXEC` 为 true，`fstat(fd)` 必须是 directory
且 identity exact 等于 `(st_dev,st_ino)`；`absolute_path` fresh
`O_RDONLY|O_DIRECTORY|O_NOFOLLOW|O_CLOEXEC` reopen也必须得到同一 identity。
lease class constructor不对 caller开放；只有本节两个 factory能返回实例。

`NamespaceProfileV1` 是 discriminated typed union，而不是一个可自由填 map：

```text
_ProductionNamespaceProfileV1 exact fields:
  mode
  repo_root_lease
...
_FixtureNamespaceProfileV1 exact fields:
  mode
  repo_root_lease
  fixture_base_lease
  isolation_root_lease
  fixture_id
  issuance_request_path
  ticket_path
  result_path
...
`repo_root := repo_root_lease.absolute_path`。fixture另定义
`isolation_root := isolation_root_lease.absolute_path`。所有 `*_path` 是 absolute
`Path`，在 factory return前冻结。

production 唯一 factory `_production_namespace_profile_v1()`：

```text
mode            = PRODUCTION_RESERVED_V23
repo_root_lease = no-follow lease of current fixed REPO_ROOT,
                  owner_created=false
issuance_request_path
...
fixture fixed base 不再需要 provisioning；它是仓库中已存在的：

```text
fixture_base = repo_root/platform/tests
```

fixture 唯一 factory
`_create_fixture_namespace_profile_v1(fixture_id)` 的 exact steps：

```text
1. create repo_root_lease exactly as production factory;
2. from that fd walk exact components platform/tests with
   O_RDONLY|O_DIRECTORY|O_NOFOLLOW|O_CLOEXEC;
3. freeze fixture_base_lease with owner_created=false;
4. require fixture_id is exact one component matching
   fixture-[0-9a-f]{24};
5. mkdirat(fixture_base_lease.fd, fixture_id, 0700);
   EEXIST is hard failure; no reuse/adoption;
6. immediately openat the new name with
   O_RDONLY|O_DIRECTORY|O_NOFOLLOW|O_CLOEXEC;
7. fstat the fd, require directory and st_nlink>=2, then freeze
   isolation_root_lease with owner_created=true;
8. no-follow stat the name through fixture_base_lease.fd and require
   (st_dev,st_ino)=isolation_root_lease identity;
9. derive the profile paths below and return the exact typed object.
```

returned fixture profile：

```text
mode            = ISOLATED_DEFINITION_FIXTURE_V1
repo_root_lease = factory step 1 lease
fixture_base_lease = factory step 3 lease
isolation_root_lease = factory step 7 owned lease
fixture_id      = exact input from step 4
...
private seam 入口、每次 fixture dynamic child walk前后、`S_1`、`S_2`和 return前
必须同时：

```text
fstat all three lease fds = their frozen identities
fresh no-follow walk repo_root/platform/tests = fixture_base identity
no-follow stat(fixture_base_fd, fixture_id) = isolation_root identity
```

fixture dynamic paths从 `isolation_root_lease.fd` 起走；bound production/read-only
artifacts仍从 `repo_root_lease.fd` 起走。任一 mismatch失败。factory 创建 root
后发生的 failure不自动删除该 directory；它被标为 poisoned fixture并留给明确
的测试清理账本。本 schema不对 active malicious same-user pathname swap声明
conditional-delete或自动清理安全性。
...
5. profile factory只以
   `os.open(repo_root, O_RDONLY|O_DIRECTORY|O_NOFOLLOW|O_CLOEXEC)` 建立
   `repo_root_lease`；`lstat(repo_root)` 必须是 directory且其
   `(st_dev,st_ino)` exact 等于 lease。seam不按 path另猜 root，而是验证并使用
   lease fd。所有 parent component 从相应 root/isolation lease fd逐级
...
待创建而尚不存在的 Q/ticket/result/marker只验证 parent chain和 lexical name；
存在性规则由相应阶段单独校验。G6 使用无 pathname 的 `O_TMPFILE`，见第 9 节。
...
bound_artifacts
  := complete pre-Q exact map from §2.4; every file already exists;
     ...
     pre-plan seed/candidate/SourceMap_B validation in §3.1
implementation_identity
...
pre-anchor authority 是一个已存在、strict pretty JSON artifact
`ImplementationIdentityCoreV1`。top-level exact fields：
...
```text
artifact=actual_wake_reachable_pressure_v23_implementation_identity_core
schema_version=1.0
version=S3ai-v2.3-implementation-identity-core-v1
source_kind=external_implementation_orchestrator_trace_identity
```

五个 identity strings 必须 nonempty。该 core由 implementation orchestrator在
source trace、anchor、candidate bound artifacts、plan和 Q之前一次性写入并
raw-freeze；它不含 source-trace path/hash、bound-artifact digest、Q/M/A/C/Z或
ticket字段。`bound_artifacts.implementation_identity_core` 对其 path/raw SHA
提供唯一 binding。G5 reviewer以后用独立 Z trace核验 cross-family关系，但不得
回写 core。
...
该 body 的五个 identity fields exact 复制已冻结 core；另两字段只在 source
trace写完后加入。G5 前冻结的 anchor 只是该 body 的 envelope，不是五字段的
producer。ticket、projection、Q、M、A 中的 nested
`implementation_identity` 必须逐字段 exact等于 `anchor.identity`，不得与
anchor envelope本身比较。
...
`g6_otmpfile_capability_probe` 必须 exact 绑定
`platform/docs/diag/actual_wake_reachable_pressure_g6_otmpfile_capability_probe_20260728_213611.json`
及 raw SHA
...
`implementation_identity_core_path/raw_sha256` 必须 exact 等于
`bound_artifacts.implementation_identity_core`。五个 identity fields的因果
producer只允许是已 strict-parse/raw-bind 的 core，并必须 exact 等于 core同名
字段；anchor随后也从同一 core复制，故 source trace不引用 future anchor。
...
顺序。pre-plan producer顺序唯一为：

```text
0. identity_core:
   strict read the already-existing bound implementation identity core;
   freeze its five identity fields as IdentityCore_B.

1. source_observations:
   no-follow read the three already-existing files;
   make exact {path,raw_sha256} bindings.

2. SourceMap_seed:
   construct a new exact object from source_observations in the three-key
   schema above; it does not contain anchor, source-trace or future artifact.

3. implementation source trace:
   five identity fields := IdentityCore_B;
   core path/raw SHA := bound core path/raw SHA;
   source_artifact_map_sha256
     := SHA256(canonical JSON of SourceMap_seed);
   write/freeze the source trace.

4. implementation anchor:
   first five anchor.identity fields := IdentityCore_B;
   bind the exact source-trace path/raw SHA in anchor.identity;
   write/freeze the anchor.

5. candidate_bound_artifacts:
   observe and strict-validate the complete §2.4 map, now including the
   already-existing source trace and anchor bindings.

6. SourceMap_candidate:
   positive-project the exact three keys from candidate_bound_artifacts;
   require SourceMap_candidate = SourceMap_seed.

7. plan:
   plan.bound_artifacts := frozen deep copy(candidate_bound_artifacts);
   SourceMap_B := positive projection of plan.bound_artifacts;
   require SourceMap_B = SourceMap_candidate = SourceMap_seed.
```

`IdentityCore_B`、`SourceMap_seed` 与 `candidate_bound_artifacts` 是 typed
in-memory observations，不是 plan字段；core本身是 pre-anchor bound artifact。
因此没有“先有 anchor才知道 trace identity”或“先有 plan再验证 plan input”的
循环。loader verification从已经存在的 core、三个 source files、source
trace、anchor和 complete candidate重复上述 0–7；不写文件。
...
accepted final-ticket verification:
  ...
  SourceMap_T = SourceMap_B = SourceMap_seed
  TKT.bound_artifacts.implementation_identity_core
    = candidate bound core binding
  TKT/anchor/source-trace five identity fields = IdentityCore_B
  SHA256(canonical JSON of SourceMap_T)
    = source trace source_artifact_map_sha256

rejected:
  validate SourceMap_seed/candidate/SourceMap_B only;
  TKT and SourceMap_T do not exist
```

构造 plan/Build/Q 之前，G5 pre-Q builder必须完成步骤 1–7。accepted loader在
TKT出现后执行 SourceMap_T 独立投影；等值由 `P(TKT)=Build` 间接且由本节
direct equality同时证明。任何分支都不得等待、预测或反向读取 future ticket。
...
expected_size  := len(_pretty_json_bytes(TKT))
```

`canonical_name` 必须是单一 POSIX component，无 `/`、`.`/`..` 或 alternate
spelling。G6 不再创建 named temp，不执行 pathname cleanup，也不使用
hard-link-then-unlink。机制选择由已绑定 capability probe证明：

```text
O_TMPFILE open = PASS
AT_EMPTY_PATH direct link = FAIL ENOENT
/proc/self/fd/<fd> + linkat(..., AT_SYMLINK_FOLLOW) = PASS
linked inode identity equal = true
post-link nlink = 1
```

Linux constants/source path exact：

```text
AT_FDCWD          := -100
AT_SYMLINK_FOLLOW := 0x400
proc_source(tmp_fd)
  := ASCII bytes "/proc/self/fd/" + base-10 tmp_fd with no sign/padding
```

`/proc/self/fd` 必须存在；`proc_source` no-follow lstat必须是 symlink，followed
stat必须 exact 等于 `fstat(tmp_fd)`。若 O_TMPFILE、procfs或 linkat primitive
不可用/不匹配，fail closed；禁止 named-temp fallback。

production issuance只接受 production profile + accepted C；fixture profile只
允许同算法 fault tests，不释放 production bearer、不启动 history。G6 exact
state machine：

```text
S0 PINNED
  validate namespace profile leases per §0.1
  walk lexical ticket parent and pin ticket_dir identity D
  require canonical_name ENOENT

S1 UNNAMED_CREATED
  openat(ticket_dir_fd, ".",
    O_RDWR|O_TMPFILE|O_CLOEXEC, 0400) -> tmp_fd
  immediately fstat(tmp_fd)
  require regular, st_nlink=0, st_size=0
  created_identity := (st_dev,st_ino)

S2 UNNAMED_COMPLETE
  write exact pretty ticket bytes; reject short/extra write
  fsync(tmp_fd)
  require fstat regular, same created_identity, st_nlink=0,
    st_size=expected_size
  pread exact expected_size+1 and require exact raw bytes then EOF
  complete_identity := (st_dev,st_ino,expected_size)

S3 PRELINK_VERIFIED
  perform full Q/M/A/C/Z/projection/credential revalidation
  validate all profile leases and re-walk ticket parent
  require re-walk parent (st_dev,st_ino)=D
  require canonical_name ENOENT
  require tmp_fd still complete_identity, nlink=0 and exact raw bytes
  require proc_source lstat symlink and followed stat=complete_identity

S4 LINKED
  rc := libc.linkat(
    AT_FDCWD, proc_source(tmp_fd),
    ticket_dir_fd, canonical_name,
    AT_SYMLINK_FOLLOW)
  only rc=0 enters LINKED; any nonzero return enters the failure table below
  fsync(ticket_dir_fd)
  require tmp_fd and no-follow canonical are regular
  require both exact same complete_identity and st_nlink=1
  no-follow open/read canonical bytes; require raw/canonical/self digest exact

S5 CANONICAL_VERIFIED
  validate all profile leases and re-walk ticket parent
  require re-walk parent identity=D
  require canonical name resolves to complete_identity, st_nlink=1
  require canonical raw/canonical/self digest unchanged
  close tmp_fd; reopen canonical no-follow
  require same complete_identity, st_nlink=1 and exact bytes

S6 RELEASED
  only after every S5 equality passes, release T once to custodian
```

custody invariant：

```text
released_to_custodian := false initially
outer finally:
  if released_to_custodian is false:
    destroy broker custody first;
    record custody_destroyed=true;
  no filesystem assertion/close/re-walk exception may bypass this finally
```

`released_to_custodian=true` 只能作为 S6 的最后一个 state write，在 custodian
已成功接受 T 后发生；此前任何 return/raise均触发 destruction。

failure table：

```text
A. failure before linkat call, including S0 canonical preexistence:
   outer finally destroys custody first
   close tmp_fd if created
   validate leases/re-walk and observe canonical without touching it
   if close succeeded AND re-walk succeeded AND canonical is ENOENT:
     CLEAN_PRELINK_ABORT_NO_NAMESPACE
   else:
     AUTHORIZATION_NAMESPACE_POISONED

B. linkat rc != 0, including EEXIST:
   outer finally destroys custody first
   close tmp_fd
   validate leases/re-walk and observe canonical without touching it
   if close succeeded AND re-walk succeeded AND canonical is ENOENT:
     ...
     AUTHORIZATION_NAMESPACE_POISONED
   if canonical exists and is same complete_identity, treat as
     AMBIGUOUS_COMMITTED_POISONED; preserve it
   if canonical exists and is any other identity, treat as
     FOREIGN_CANONICAL_COLLISION_POISONED; preserve it

C. linkat rc == 0 then any S4/S5/launch failure:
   outer finally destroys custody first
   close tmp_fd if open
   AUTHORIZATION_NAMESPACE_POISONED
   preserve canonical in its observed state
```

close、absence observation或re-walk本身失败只能把 clean abort升级为 poison，
不能跳过 custody destruction。任何 failure branch均不能进入 S6。

这里没有 pathname unlink、rename或cleanup authority。`linkat rc=0` 是唯一
commit boundary；publisher永不删除、不覆盖 canonical，poison后另开 successor。
本 profile不声称对 canonical link创建后的 active malicious writer或 pre-marker
token复制提供 durable protection。
...
- production/fixture typed lease factory、fixture exclusive mkdir/inode持续核验、
  Q profile ownership、双向component-prefix隔离、exact repo-root/path/run-dir；
- SourceMap seed→candidate→plan与 accepted T/rejected no-TKT因果顺序；
- IdentityCore→source trace→anchor五字段逐项相等及任一 future-anchor引用拒绝；
...
- strict bearer；
- G6 O_TMPFILE `nlink=0→1`、partial write、proc-fd source identity、EEXIST
  no-replace、parent re-walk、close-on-prelink-failure与每阶段 fault injection；
- G6 S0 collision、linkat nonzero canonical absent/same inode/foreign inode、
  close/re-walk/observation failure均先销毁 custody且永不到 S6；
- assert G6 从不创建/rename/unlink named temp；post-link failure保留 canonical并
  标记 poisoned；
