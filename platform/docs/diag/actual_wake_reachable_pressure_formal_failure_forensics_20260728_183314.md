...
本次 one-shot 不是科学意义上的 `PROTOCOL-NO-GO`，也不是源文件在运行期间被
修改。它在完成 31-history collector、aggregation 和
...
wrapper、guard、62-file source closure 和所有 claim-runtime 源当前 SHA256
均与授权一致。`find` 也未发现任何 formal Python source 在 attempt 开始后改变
mtime。
...
所以 error message 中的“set or bytes changed”可进一步定位为 **set grew**，
不是原有文件 bytes/identity 改变。
...
```text
all 63 bound repository sources unchanged
all 170 import-time external sources unchanged
