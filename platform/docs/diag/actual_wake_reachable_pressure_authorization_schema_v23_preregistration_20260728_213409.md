...
**时间**：2026-07-28T21:34:09+08:00  
**artifact**：`actual_wake_reachable_pressure_authorization_schema_v23_preregistration_20260728_213409`  
...
`SourceMap_seed` 与 `candidate_bound_artifacts` 是 typed in-memory observations，
不是 artifact或 plan字段；没有“先有 plan再验证 plan input”的循环。loader
verification从已经存在的三个文件、source trace、anchor和 complete candidate
重复上述 1、2、5、6、7；不写文件。
...
S4 LINKED
  libc.linkat(
    ...
    AT_SYMLINK_FOLLOW)
  EEXIST or any nonzero return is failure; no overwrite/fallback
  fsync(ticket_dir_fd)
...
S1 到 linkat成功前的任一 failure是 `PRELINK_FAILURE`：

```text
close tmp_fd if created
→ unnamed inode is reclaimed by kernel
→ require canonical_name remains ENOENT
→ destroy broker custody
```

这里没有 pathname unlink、rename或cleanup authority。linkat成功是唯一 commit
boundary。S4以后任何 hash、nlink、fsync、parent re-walk或 launch异常都转为
`AUTHORIZATION_NAMESPACE_POISONED`；不删除、不覆盖 canonical，broker销毁
custody并另开 successor。只有 S6可以释放 token。本 profile不声称对 canonical
link创建后的 active malicious writer或 pre-marker token复制提供 durable
protection。
