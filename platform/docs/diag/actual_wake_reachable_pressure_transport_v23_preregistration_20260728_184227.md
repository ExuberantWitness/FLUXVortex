...
**预注册时间**：2026-07-28T18:42:27+08:00  
**attempt id**：`S3ai-v2.2-transport-v2.3-successor-20260728_184227`  
...
dependency manifest:
  platform/docs/diag/actual_wake_reachable_pressure_dependency_closure_v23_20260728_184227.json
authorization:
  platform/docs/diag/actual_wake_reachable_pressure_execution_authorization_v23_20260728_184227.yaml
result:
  platform/docs/diag/actual_wake_reachable_pressure_obstruction_results_s3ai_v22_transport_v23_20260728_184227.json
marker:
  platform/docs/diag/actual_wake_reachable_pressure_obstruction_results_s3ai_v22_transport_v23_20260728_184227.json.lock
interpretation contract:
  platform/docs/diag/actual_wake_reachable_pressure_result_interpretation_contract_v23_20260728_184227.{md,json}
```
...
4. 从 `sys.modules` 移除已见 member 不能抹去证据；end-loaded set 必须与
   monotonic ledger 自洽。
...
路径 hash 和 start/end 检查只能发现 checkpoint drift，不能排除在 import
窗口“替换—加载—恢复”的 TOCTOU。**新 authorization ticket 创建前必须二选一
取得证书**：

1. audited verified loader：以 `O_NOFOLLOW` fd 读取、hash，并从同一份持有
   bytes 编译；逐次记录 Python/native load；或
2. repo、解释器、packages 和 native libraries 全部位于
   content-addressed read-only execution image。

否则 G5/G6 保持 `BLOCKED`。append-only import ledger 必须记录成功、失败、
部分初始化、瞬时、移除和 alias import，不能只观察最终 `sys.modules`。
...
12. old marker hash 和 old result/latest absence 在 fake run 前后保持；
...
20. failed/partially-initialized import 仍进入 attempted ledger；
21. 未授权 alias/module-distribution identity 必须失败。
...
| G3 no-history closure audit | clean bootstrap 重新形成并审计逐文件 `B/U/R`；给出 verified-loader 或 readonly-image certificate | `LOCKED` |
| G4 definition tests | 18 类 fake/no-history negative controls 全过 | `LOCKED` |
| G5 independent bounded audit | 逐 hash 绑定 wrapper、prereg、manifest、runtime、63 sources、registry、definition chain、new paths、old lineage、new token commitment，并接受加载边界证书 | `LOCKED` |
