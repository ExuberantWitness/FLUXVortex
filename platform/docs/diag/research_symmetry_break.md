...
**候选 C(2026-07-24)——双通道同时优于 v4,立项以来首次**:
配置 = 全矢量 Kirchhoff 砍量 + ds 涡升增强(z-only, mem Tv=6, cds=1.5)+ CT 吸力一致簿记
(−(1−√f2)²/4·ηCLaα²)+ sep_drag(Hoerner 1.20 形状阻)。每组件独立物理锚定。

| aoa | L cand | exp L | 斜率 cand/exp | T cand | exp T | v4 T |
|---|---|---|---|---|---|---|
| 0 | 2.44/3.34 | 2.88/2.95 | +0.75/+0.06 | −0.19/+0.22 | −1.41/+0.51 | −0.25/+1.37 |
| 5 | 5.56/6.68 | 6.15/7.82 | +0.93/+1.39 | −0.36/−0.20 | −2.29/−0.55 | −0.34/+1.13 |
| 15 | 11.44/13.83 | 12.87/15.15 | +2.00/+1.90 | −2.81/−3.46 | −3.79/−2.93 | −1.93/−0.89 |

12 点记分:升力 MAE **0.89 vs v4 1.10**(−19%);推力 MAE **0.88 vs v4 1.59**(−45%);
dL/df 三族符号全正(v4 全负/平),aoa15 斜率 +2.00 vs +1.90(5% 过)。残余:aoa0 升力
门控泄漏(+0.75 vs +0.06,|A0| 越 crit 但实测无动态失速——k 超 L-B 验证带的过分离),
aoa5 斜率欠 33%。

**红线补全(候选 C,7 工况)——快环过门,118 全扫启动**:
| 工况 | L cand/exp/v4 | T cand/exp/v4 |
|---|---|---|
| aoa10 f1.4/f2.6 | 8.62/10.27 (exp 10.53/12.36) | −1.11/−1.36 (exp −2.94/−1.65, v4 −0.83/+0.33) |
| U6 f1.4/f2.6 | 3.30/4.51 (exp 3.81/3.87) | −0.16/−0.30 (exp −3.17/−1.80, v4 +0.08/+1.15) |
| U10 f1.4/f2.6 | 8.52/9.34 (exp 8.64/11.37) | −0.73/−0.35 (exp −1.14/+0.74, v4 −0.96/+0.66) |
| tw45 f2.6 | 7.45 (exp 6.47, v4 6.88) | −1.62 (exp −2.05, v4 +0.70) |

19 点总账:推力 MAE ~1.02 vs v4 ~1.83(−44%);升力 ~1.03 vs ~1.04(平)。dL/df 全正
(aoa10 +1.38 vs +1.53);dT/dU 不再反号(平 vs v4 负、实测 +0.63,部分回收);tw45 深顺
桨推力 −1.62(v4 +0.70,实测 −2.05)——红线守住。弱项:aoa10 升力低 2N、U6 升力高 0.6、
aoa0 门控泄漏。→ 118 全扫(lb_sweep118.py,resumable s6_sweep_candC.json)+ trend_metrics
官方记分卡 = 晋升门终判。

**尾迹长度敏感性(2026-07-25,用户问"尾涡是否有效铺开至 5-10 展长")**:
生产尾迹 = wake_rows=spc = 恰好 1 扑动周期(1.9-3.6 展长,未到 5-10)。直测(U8/f2.6,
v4 BASE,wake 1×/2×/4× = 1.9/3.8/7.7 展长):**推力免疫(Δ≤0.14)**;升力有界振荡
±4%(aoa5 Δ0.31、aoa15 Δ0.60,非单调 15.59→15.92→15.32)——远尾迹周期交替涡环部分
对消,截断误差有界非系统。**判定:1 周期截断可用,不升 5-10 展长(16× 算力换 ≤4%
振荡带不值);aoa15 升力不确定带 ±0.6N 入诚实记录。**

**通道份额分解(2026-07-25,用户问"非 VLM 部分提供多少升力/阻力",U8/f2.6 风轴)**:
升力:v4 ≈100% UVLM(bern 101-104%);候选C bern 101-113%,修正层净额 −3%(aoa5)/−20%
(aoa15)——**量值仍是 UVLM 的数,但失速后形状全由修正层对消产生**(aoa15:砍量 −44% +
涡升回填 +20%,毛幅 2/3 升力过修正层)。阻力(毛通道):v4 修正层 17-43%(欠账)→ 候选C
52-66%(型阻/形状阻/CT/支架簿记补齐,推力 MAE −43% 的来源)。净推力在各版本均为
"吸力 +4.6 vs 阻力 ~−5"的对消差(扑动推进物理本质,非修正伪影)。

**T3b 重定位(2026-07-25,dT/df 病的真正结构)**:从 118 数据反解 v4 vs 实测推力缺口:
aoa5/10/15 缺口 1.9-2.1N **f 无关、aoa 弱相关**(aoa0: +1.16@f1.4 / −0.86@f2.6)——正是
T3b 案卷的"+2.2N@U8 普适偏置"(f/tw 无关、机理未明、用户裁定**禁常数吸收**)。候选 C 的
CT+sep_drag(f² 增长)是以错误形状部分吸收 T3b 偏置:量级蒙对(推力 MAE −43%)、形状必错
(dT/df r 0.995→−0.03)。→ **候选 D = 候选 C 撤 sep_drag**:推力回 v4 带(dT/df 形状恢复),
T3b 偏置如实挂账(开放案非模型错误)。文献佐证:粘性摩擦应 ∝f^1.5(层流 Cf∝Re^-1/2),
任何形式阻 ∝V²·sin²(arel) 的 f² 增长都过陡(Faure 2023:LDVM 在 k<1.5 必须接 static
polar C_D(α_eff),否则推力系统性高估 ~25%)。

**候选 E2(2026-07-25)——通道解耦架构验证成功**:
升力 = L-B wind-lift 方向闭合(砍量+ds-z 沿 (−sinα,0,cosα) 施加,T_wind 贡献恒为零);
推力 = 100% v4(CT 关、sep_drag 关、chop-x 关)。实测:推力全攻角逐位 v4(Δ≤0.16),
dT/df 斜率全保(+1.35/+1.22/+0.96/+0.89 vs v4 同值);升力 dL/df 四族全正(+0.78/+0.78/
+1.10/+1.66)。残余:升力量级 aoa10/15 砍量过深(MAE 1.40 vs v4 1.10)——cds=1.5 偏保守,
文献锚 C_ds 标称 3(带 [1,8]),微调 2.5 验证中。架构演化链:C(全矢量+CT+sep_drag:
推力 3×劣化)→ D(撤 sep_drag:CT f² 压平斜率)→ E(chop-z:sin(aoa) 泄漏 +1.5N)→
**E2(wind-lift 方向:零泄漏)**——每步都有通道分解证据,非盲试。

**cds=2.5 微调(E2 架构)+ ds 方向再归因(2026-07-25)**:
cds=2.5(文献锚标称 3 带内):升力 MAE **0.62 vs v4 1.10**(−44%),aoa5 近完美
(5.91/7.91 vs 6.15/7.82)、aoa15 f1.4 完美(12.97 vs 12.87)、aoa10 −1.2/−0.7;
斜率全正但系统性 +25% 过(aoa15 +2.39 vs +1.90)。唯一病灶 = **aoa0 泄漏**(2.75/4.77,
斜率 +1.69 vs +0.06)。
**ds 方向再归因(纠错)**:此前"ds x 投影 +3~7N"系错归因(真犯=砍量 x 投影与 CT 双计,
均已查明);ds 自身 x 投影仅 ~0.06-0.3N(扭转 ±11° 前倾角小)。z-only ds 是过度修正。
物理:涡力应沿**截面法向**——下行程 +升力/上行程 −升力(aoa0 对称相消,实测 +0.06 ✓),
法向前倾产生小幅 LEV 前推(实测 tw22.5 脉冲 +0.73N 与"推力随 f 升"的物理源)。
→ 候选 F = 砍量 wind-lift + ds 截面法向,验证中。

**候选 F(cds=2.5, ds 截面法向)——涡诱导阻力自然捕获 T3b 方向(2026-07-25)**:
16 点:升力 MAE **0.56**(E2 0.62、v4 1.10);推力 MAE **1.09**(E2/v4 1.59)——aoa15
推力 −3.36/−2.79 贴实测(−3.79/−2.93,误差 +0.43/+0.14):高攻角法向后倾 = 涡升力
伴随的涡诱导阻力(真实物理),f 增长温和(+11%/f),方向即 T3b 偏置方向。代价:dT/df
斜率 aoa10/15 略平(+0.64/+0.48 vs v4 +0.97/+0.87,实测 +1.08/+0.72 居中)。aoa0 泄漏
不变(幅度驱动根因,候选 G 带符号驱动专攻)。

**绘图 L/T 解包 bug 修复(2026-07-25,诚信教训)**:三者对比图(fig17/18/19_3way.png)
`T4,L4=zip(*v4)` 把 mod 返回的 (L,T) 赋反——升力子图画成推力(值−5~5)、推力子图画成
升力,实测线却正确,导致用户看到"实测6N vs 模型≈0、误差400%"的惊吓画面。**数据与模型
全程正确**(数据表+视觉模型验单曲线图 aoa15 v4.1=12.78-15.59N 为证)。中途误判怪中文字体
是错(道歉)。修复=显式 `v["L"]`/`v["T"]` 取值(platform/plot_3way.py 固化)。教训:
1)解包禁用 `a,b=zip(*)` 隐式,用显式索引;2)交付图必须视觉模型核曲线位置,禁基于数据假设。
