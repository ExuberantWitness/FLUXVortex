"""No-GT focused tests for the v5h13 B3 coupling transaction."""
...
os.environ.setdefault("NUMBA_CACHE_DIR", "/tmp/fluxv-v5h13-coupling-numba")
os.environ.setdefault("MPLCONFIGDIR", "/tmp/fluxv-v5h13-coupling-mpl")
...
import forward_flight_benchmarks.fluxv_v5h13_baik_coupling as coupling
...
class _NativeMockCommitter:
    ...
    def __init__(self) -> None:
        self.requests: list[V5H13GlobalRowCommitRequest] = []
        self.commits: list[RowCommitResult] = []

    def __call__(self, request: V5H13GlobalRowCommitRequest) -> V5H13RowCommitEnvelope:
        ...
        row = make_release_row(
            ...
            sheet_id="baik-w2-v5h13-synthetic-row",
        )
        if request.previous_owner is None:
            owner = bootstrap_release_row_owner(
                ...
                particle_cap=1000,
                owner_id=f"v5h13-native-synthetic:{next(_OWNER_INDEX)}",
            )
            ...
            proposal = propose_release_row_update(
                ...
                row,
                proposal_id=f"v5h13-native-release-{release}",
            )
...
def _native_config() -> V5H13BaikCouplingConfig:
    return V5H13BaikCouplingConfig(
...
def _owner():
    ...
    row = make_release_row(
        ...
        release_index=1,
        source_time_s=1.0,
        sheet_id="v5h13-synthetic-sheet",
    )
    return bootstrap_release_row_owner(
        ...
        release_dt_s=_DT,
        particle_cap=10_000,
        owner_id=f"v5h13-synthetic:{next(_OWNER_INDEX)}",
    )
...
def _load_ledger(*, forces_source: np.ndarray | None = None):
    ...
    return make_v5h13_layer_load_ledger(
        ptera_step_index=3,
...
def _config(n: int = 1) -> V5H13BaikCouplingConfig:
    return V5H13BaikCouplingConfig(
...
def _kelvin(owner, *, residual: float = 0.0):
    return make_v5h13_source_kelvin_evidence(
        source_step_index=4,
...
def test_config_separates_formal_matrix_diagnostic_smoke_and_synthetic_small_n() -> (
    ...
    with pytest.raises(ValueError, match="exactly one"):
        V5H13BaikCouplingConfig(transport_substeps=64)
    with pytest.raises(ValueError, match="exactly one"):
        V5H13BaikCouplingConfig(
    ...
    with pytest.raises(ValueError, match="formal B3 N"):
        V5H13BaikCouplingConfig(transport_substeps=1, formal_matrix=True)
    with pytest.raises(ValueError, match="small-test cap"):
        V5H13BaikCouplingConfig(transport_substeps=32, test_mode=True)
    ...
    assert (
        V5H13BaikCouplingConfig(transport_substeps=64, formal_matrix=True).role
        == "candidate"
    )
    diagnostic = V5H13BaikCouplingConfig(
        transport_substeps=32,
        diagnostic_smoke=True,
        active_layer_limit=1,
    )
...
def test_native_one_layer_orders_commit_before_parent_and_closes_exact_ledgers(
    ...
    def guarded_parent_wake(self: object) -> object:
        ...
        if step in ACTIVE_PTERA_STEPS:
            ...
            assert self._v5h13_active_owner is committer.commits[-1].owner  # type: ignore[attr-defined]
    ...
    class _OrderedCommitter:
        def __call__(
            self, request: V5H13GlobalRowCommitRequest
        ) -> V5H13RowCommitEnvelope:
    ...
    solver = make_fluxv_v5h13_baik_w2_solver(
        _native_problem(),
    ...
    assert solver.v5h13_slice_complete
    assert len(solver.v5h13_layer_results) == 1
    result = solver.v5h13_layer_results[0]
...
def test_native_transport_failure_keeps_owner_current_and_no_success_record(
    ...
    assert solver.v5h13_layer_results == []
    assert len(solver.v5h3_feedback_step_reports) == 1
    assert not solver.v5h13_slice_complete
...
def test_load_ledger_is_detached_readonly_and_recomputed() -> None:
    ...
    assert validate_v5h13_layer_load_ledger(ledger) is ledger
    with pytest.raises(ValueError, match="no-penetration"):
        validate_v5h13_layer_load_ledger(replace(ledger, no_penetration_max_abs=1.0))
    with pytest.raises(ValueError, match="native counts"):
        validate_v5h13_layer_load_ledger(replace(ledger, load_batch_evaluation_count=3))
...
def test_small_n_layer_closes_stream_fd_support_owner_and_scope() -> None:
    ...
    with pytest.raises(ValueError, match="child digest"):
        validate_v5h13_layer_result(replace(result, fd_ledger_sha256="0" * 64))
...
def test_parent_failure_leaves_row_owner_unadvanced_and_returns_no_result() -> None:
    ...
    assert owner.state.phase == "post_commit_pre_transport"
    assert owner.transport_events == ()
    retry = transport_v5h13_committed_layer(
        owner,
        config=_config(1),
        ptera_step_index=3,
        source_step_index=4,
    ...
    assert validate_v5h13_layer_result(retry) is retry
...
def test_layer_result_copy_is_not_live_issued() -> None:
    ...
    with pytest.raises(ValueError, match="exact live issued"):
        validate_v5h13_layer_result(replace(result))
...
def test_external_manual_publish_lacks_the_private_issuance_role() -> None:
    ...
    assert not hasattr(coupling, "_transport_v5h13_committed_layer_impl")
    assert not hasattr(coupling, "_make_live_layer_result_registry")
    assert transport_v5h13_committed_layer.__defaults__ is None
    assert transport_v5h13_committed_layer.__kwdefaults__ is None
    assert transport_v5h13_committed_layer.__closure__ is None
...
def test_graded_birth_window_synthetic_layer_uses_effective_substeps() -> None:
    ...
    owner6 = _owner()
    result6 = transport_v5h13_committed_layer(
        owner6,
        config=graded6,
        ptera_step_index=3,
        source_step_index=4,
