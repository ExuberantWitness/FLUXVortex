"""Protocol-only tests for the v5h12 Baik-W2 no-observation runner.
...
_RUNNER_PATH = (
    ...
    / "run_fluxv_v5h12_baik_w2.py"
)
_SPEC = importlib.util.spec_from_file_location(
    "fluxv_v5h12_w2_runner_under_test", _RUNNER_PATH
)
...
def _pristine_audited_runtime_modules():
    ...
    joint pytest process (e.g. the frozen V5H11 coupling tests) must not leak
    ...
    *,
    ...
    for name in runner.REQUIRED_DEPENDENCY_LEAVES:
        if name == "formal_executor_module":
            path = (tmp_path / "fluxv_v5h12_baik_w2_executor.py").resolve()
            path.write_text(
                executor_source
                or "def build_fluxv_v5h12_w2_executor(api):\n    return object()\n",
                encoding="utf-8",
            )
            canonical_paths[name] = str(path)
        elif name == "formal_executor_test":
            path = (tmp_path / "test_fluxv_v5h12_baik_w2_executor.py").resolve()
            path.write_text(
...
def pass_pair(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    root = tmp_path_factory.mktemp("v5h12_w2_pass")
...
def test_dependency_drift_after_preflight_publishes_explicit_stop(
    ...
    executor_source = """
from pathlib import Path
def build_fluxv_v5h12_w2_executor(api):
...
def test_audited_executor_must_not_import_runner_class_copy(
    ...
    token_path, _, _ = _make_external_audit_token(
        ...
        monkeypatch,
        executor_source=(
            "import forward_flight_benchmarks.run_fluxv_v5h12_baik_w2\n"
            "def build_fluxv_v5h12_w2_executor(api):\n"
            "    return object()\n"
        ),
    )
...
def test_audited_executor_importfrom_alias_cannot_import_runner(
    ...
    token_path, _, _ = _make_external_audit_token(
        ...
        executor_source=(
            ...
            "run_fluxv_v5h12_baik_w2 as runner\n"
            "def build_fluxv_v5h12_w2_executor(api):\n"
            "    return object()\n"
        ),
    )
...
def test_verify_recomputes_summary_semantic_input_map(
    ...
    summary["summary_payload_sha256"] = runner._payload_sha256(
        "fluxv-v5h12-baik-w2-summary-v1", summary_without_hash
    )
...
def test_source_append_failure_blocks_stages_and_clean_retry_succeeds() -> None:
    ...
    assert len(clean.transport_stages) == 3 * 32
...
def test_commit_rejects_same_key_source_with_different_canonical_bytes() -> None:
    ...
    assert len(sink.source_events) == 1
    assert len(sink.transport_stages) == 3 * 32
    assert not sink.raw_steps
