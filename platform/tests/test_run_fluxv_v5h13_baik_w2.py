"""Protocol-only tests for the v5h13 Baik-W2 no-observation runner.
...
_RUNNER_PATH = (
    ...
    / "run_fluxv_v5h13_baik_w2.py"
)
_SPEC = importlib.util.spec_from_file_location(
    "fluxv_v5h13_w2_runner_under_test", _RUNNER_PATH
)
...
def _pristine_audited_runtime_modules():
    ...
    joint pytest process (e.g. the frozen V5H13 coupling tests) must not leak
...
def _owner_event(level: int, layer: int) -> dict[str, object]:
    ...
    row.update(
        {
            "schema_id": "fluxv-v5h13-baik-w2-owner-event-v1",
            "transport_substeps": level,
            "layer": layer,
    ...
    return {
        ...
        "metadata": {
            ...
            "source_prehistory_manifest_sha256": runner._payload_sha256(
                "fluxv-v5h13-source-prehistory-manifests-v1", prehistory
            ),
            "source_prehistory_manifests": prehistory,
            "source_kelvin_ledger_sha256": runner._payload_sha256(
                "fluxv-v5h13-source-kelvin-ledgers-v1",
    ...
    *,
    ...
    for name in runner.REQUIRED_DEPENDENCY_LEAVES:
        if name == "formal_executor_module":
            path = (tmp_path / "fluxv_v5h13_baik_w2_executor.py").resolve()
            path.write_text(
                executor_source
                or "def build_fluxv_v5h13_w2_executor(api):\n    return object()\n",
                encoding="utf-8",
            )
            canonical_paths[name] = str(path)
        elif name == "formal_executor_test":
            path = (tmp_path / "test_fluxv_v5h13_baik_w2_executor.py").resolve()
            path.write_text(
...
def pass_pair(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    root = tmp_path_factory.mktemp("v5h13_w2_pass")
...
def test_dependency_drift_after_preflight_publishes_explicit_stop(
    ...
    executor_source = """
from pathlib import Path
def build_fluxv_v5h13_w2_executor(api):
...
def test_audited_executor_must_not_import_runner_class_copy(
    ...
    token_path, _, _ = _make_external_audit_token(
        ...
        monkeypatch,
        executor_source=(
            "import forward_flight_benchmarks.run_fluxv_v5h13_baik_w2\n"
            "def build_fluxv_v5h13_w2_executor(api):\n"
            "    return object()\n"
        ),
    )
...
def test_audited_executor_importfrom_alias_cannot_import_runner(
    ...
    token_path, _, _ = _make_external_audit_token(
        ...
        executor_source=(
            ...
            "run_fluxv_v5h13_baik_w2 as runner\n"
            "def build_fluxv_v5h13_w2_executor(api):\n"
            "    return object()\n"
        ),
    )
