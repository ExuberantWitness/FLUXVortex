"""Protocol-only tests for the v5h15 Baik-W2 no-observation runner.
...
_RUNNER_PATH = (
    ...
    / "run_fluxv_v5h15_baik_w2.py"
)
_SPEC = importlib.util.spec_from_file_location(
    "fluxv_v5h15_w2_runner_under_test", _RUNNER_PATH
)
...
_H = "0" * 64
_ERROR = {47: 4.0e-8, 79: 1.0e-8, 143: 0.0}


_AUDITED_MODULE_PREFIXES = runner.AUDITED_RUNTIME_MODULE_PREFIXES
...
def _pristine_audited_runtime_modules():
    """Run every protocol test as if no project/Ptera module had been imported.

    These tests are protocol-only and never import the coupling stack.  The
    runner's dependency preflight/postflight attests the loaded runtime-module
    inventory, so modules legitimately imported by other suites in the same
    joint pytest process (e.g. the frozen V5H15 coupling tests) must not leak
    into this module's tests.  The runner gate itself is never relaxed; only
    the test-time process environment is restored.
    """
...
def _owner_event(level: int, layer: int) -> dict[str, object]:
    ...
    row.update(
        {
            "schema_id": "fluxv-v5h15-baik-w2-owner-event-v1",
            "transport_substeps": level,
            "layer": layer,
            ...
            row.update(
                {
                    ...
                    "stage": stage,
                    "status": "completed",
                    "substep_delta_time": runner.graded_substep_delta_time(
                        level, substep
                    ),
                    "rk_a": rk_a[stage - 1],
                    "rk_b": rk_b[stage - 1],
    ...
    return {
        ...
        "metadata": {
            ...
            "particle_id_sequence_sha256": runner._payload_sha256(
                "fluxv-v5h15-particle-id-sequence-v1", list(particle_ids)
            ),
            "material_tracer_id_sequence_sha256": runner._payload_sha256(
                "fluxv-v5h15-material-id-sequence-v1", list(material_ids)
            ),
...
def _with_force_convergence_failure(
    ...
    """Keep every layer cross-link valid while making the candidate level a bad load candidate."""
    ...
    *,
    ...
    for name in runner.REQUIRED_DEPENDENCY_LEAVES:
        if name == "formal_executor_module":
            path = (tmp_path / "fluxv_v5h15_baik_w2_executor.py").resolve()
            path.write_text(
                executor_source
                or "def build_fluxv_v5h15_w2_executor(api):\n    return object()\n",
                encoding="utf-8",
            )
            canonical_paths[name] = str(path)
        elif name == "formal_executor_test":
            path = (tmp_path / "test_fluxv_v5h15_baik_w2_executor.py").resolve()
            path.write_text(
...
def pass_pair(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    root = tmp_path_factory.mktemp("v5h15_w2_pass")
...
def test_pass_bundle_has_exact_counts_base64_and_independent_convergence(
    ...
    assert np.array_equal(
        ...
        _trajectory(47, 1)["arrays"]["end_positions"],
    )
...
def test_convergence_is_recomputed_from_raw_durable_inputs(
    ...
    record = next(
        ...
        if item["transport_substeps"] == 79 and item["layer"] == 1
    )
...
def test_dependency_drift_after_preflight_publishes_explicit_stop(
    ...
    executor_source = """
from pathlib import Path
def build_fluxv_v5h15_w2_executor(api):
...
def test_audited_executor_must_not_import_runner_class_copy(
    ...
    token_path, _, _ = _make_external_audit_token(
        ...
        monkeypatch,
        executor_source=(
            "import forward_flight_benchmarks.run_fluxv_v5h15_baik_w2\n"
            "def build_fluxv_v5h15_w2_executor(api):\n"
            "    return object()\n"
        ),
    )
...
def test_audited_executor_importfrom_alias_cannot_import_runner(
    ...
    token_path, _, _ = _make_external_audit_token(
        ...
        executor_source=(
            ...
            "run_fluxv_v5h15_baik_w2 as runner\n"
            "def build_fluxv_v5h15_w2_executor(api):\n"
            "    return object()\n"
        ),
    )
...
def test_stage_fields_bind_directly_to_compact_coupling_evidence() -> None:
    ...
    sink.add_source_event(_source_event(47, 4))
...
def test_stop_prefix_rejects_terminal_coordinate_and_commit_order_drift() -> None:
    ...
    incomplete_load = runner.ArtifactRecords(
        ...
        transport_stages=full.transport_stages[: 3 * 47],
        trajectories=full.trajectories[:1],
        terminal_coordinate={
            "transport_substeps": 47,
            "layer": 1,
...
def test_sink_atomic_layer_commit_waits_for_full_stage_block() -> None:
    ...
    sink = runner.ArtifactSink()
    sink.add_source_event(full.source_events[0])
    for row in full.transport_stages[: 3 * 47]:
        _append_compact_stage(sink, dict(row))
    sink.commit_completed_layer(
        raw_step=full.raw_steps[0],
        source_event=full.source_events[0],
...
def test_producer_rejects_hostile_completed_crosslink_evidence() -> None:
    ...
    first_stage_index = next(
        ...
        if runner._stage_key(row) == (47, 2, 1, 1)
    )
...
def test_trajectory_macro_invariant_and_b1_gamma_domain_are_replayed() -> None:
    runner._validate_completed_trajectory_semantics(_trajectory(47, 1))
    ...
    zero_sigma_drift = _trajectory(47, 1)
    ...
    signed_zero_drift = _trajectory(47, 1)
...
def test_verify_recomputes_summary_semantic_input_map(
    ...
    summary["summary_payload_sha256"] = runner._payload_sha256(
        "fluxv-v5h15-baik-w2-summary-v1", summary_without_hash
    )
...
def test_trajectory_sink_freezes_exact_encoded_array_bytes() -> None:
    ...
    completed_float32 = _trajectory(47, 1)
...
def _conversion_stop_records(
    full: runner.ArtifactRecords,
    *,
    stage_count: int,
    ...
    return runner.ArtifactRecords(
        ...
        particle_counts=(),
        raw_loads=(),
        transport_stages=full.transport_stages[:stage_count],
        trajectories=(),
        terminal_coordinate=coordinate,
        stop_code="stage_evidence_conversion_error",
        stop_message="ExecutorContractError: synthetic conversion failure",
    )


def test_conversion_stop_terminal_preserves_unbegun_next_stage_coordinate() -> None:
    full = _pass_records()
    zeroth = _conversion_stop_records(
        full,
        stage_count=0,
        coordinate={
            "transport_substeps": 47,
    ...
    runner._validate_records(zeroth)
    second = _conversion_stop_records(
        full,
        stage_count=1,
        coordinate={
            "transport_substeps": 47,
    ...
    runner._validate_records(second)
    generic = runner.ArtifactRecords(
        execution_mode=runner.SYNTHETIC_EXECUTION_MODE,
        status="STOP",
        raw_steps=(),
        source_events=full.source_events[:1],
        ...
        stop_code="formal_solver_stopped",
        stop_message="RuntimeError: synthetic",
    )
    runner._validate_records(generic)


def test_conversion_stop_rejects_hostile_coordinate_with_last_good_stop() -> None:
    sink = runner.ArtifactSink()
    sink.add_source_event(_source_event(47, 4))
    records = runner._validated_stop_from_sink(
        sink,
        stop_code="stage_evidence_conversion_error",
        stop_message="ExecutorContractError: synthetic conversion failure",
        phase="artifact_stage_conversion",
    ...
    assert records.stop_code == "coupling_stop_contract_error"
    assert records.terminal_coordinate == {
        "transport_substeps": 47,
        "layer": None,
        "source_step_index": 4,
        "ptera_step_index": 3,
    ...
    assert len(records.source_events) == 1


def test_commit_requires_pre_appended_durable_source_and_leaves_sink_unchanged() -> (
    None
):
    # A 96-stage prefix without a source parent is unreachable through the
    # public ABI: every stage append is rejected first.  The commit guard is
    # therefore exercised from the empty sink, which is the only legal state
    # that has stages absent AND its source parent absent.
    full = _pass_records()
    sink = runner.ArtifactSink()
    with pytest.raises(ValueError, match="source-event parent"):
        _append_compact_stage(sink, dict(full.transport_stages[0]))
    with pytest.raises(ValueError, match="pre-appended source event"):
        sink.commit_completed_layer(
            raw_step=full.raw_steps[0],
            source_event=full.source_events[0],
            owner_event=full.owner_events[0],
...
def test_exactly_once_contract_rejects_identical_duplicate_source_append() -> None:
    full = _pass_records()
    sink = runner.ArtifactSink()
    sink.add_source_event(full.source_events[0])
    with pytest.raises(ValueError, match="prefix"):
        sink.add_source_event(dict(full.source_events[0]))
    assert len(sink.source_events) == 1


def test_exactly_once_contract_rejects_inconsistent_duplicate_source_append() -> None:
    full = _pass_records()
    sink = runner.ArtifactSink()
    sink.add_source_event(full.source_events[0])
    inconsistent = dict(full.source_events[0])
    inconsistent["a0_pre"] = [value + 1.0 for value in inconsistent["a0_pre"]]
    with pytest.raises(ValueError, match="prefix"):
        sink.add_source_event(inconsistent)
    assert len(sink.source_events) == 1


def test_source_exactly_once_then_stages_then_commit_ends_with_one_source() -> None:
    full = _pass_records()
    sink = runner.ArtifactSink()
    sink.add_source_event(full.source_events[0])
    for row in full.transport_stages[: 3 * 47]:
    ...
    with pytest.raises(ValueError, match="prefix"):
        sink.add_source_event(dict(full.source_events[0]))
    sink.add_source_event(full.source_events[1])
    assert len(sink.source_events) == 2


def test_source_append_failure_blocks_stages_and_clean_retry_succeeds() -> None:
    full = _pass_records()
    sink = runner.ArtifactSink()
    bad_source = dict(full.source_events[0])
    bad_source["cell_count"] = 7
    ...
    clean = runner.ArtifactSink()
    clean.add_source_event(full.source_events[0])
    for row in full.transport_stages[: 3 * 47]:
        _append_compact_stage(clean, dict(row))
    clean.commit_completed_layer(
        raw_step=full.raw_steps[0],
        source_event=full.source_events[0],
        owner_event=full.owner_events[0],
        particle_count=full.particle_counts[0],
    ...
    assert len(clean.source_events) == 1
    assert len(clean.transport_stages) == 3 * 47
    assert len(clean.raw_steps) == 1


def test_first_stage_conversion_stop_publishes_and_reverifies_byte_identically(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    executor_source = """
def build_fluxv_v5h15_w2_executor(api):
    class SyntheticConversionStopExecutor:
        def attest_dependency_origins(self):
            return None
        def run_formal_matrix(self, *, levels, sink):
            row = {name: 0 for name in api.source_event_fields}
            ...
            sink.add_source_event(row)
            raise api.stop_constructor(
                "stage_evidence_conversion_error",
                {
                    "transport_substeps": 47,
                    "layer": 1,
                    "source_step_index": 4,
                ...
                "ExecutorContractError: synthetic first-record conversion failure",
            )
    return SyntheticConversionStopExecutor()
""".lstrip()
    audit_root = tmp_path / "conversion-publish-audit"
    audit_root.mkdir()
    token_path, _, _ = _make_external_audit_token(
        audit_root, monkeypatch, executor_source=executor_source
    )
    output = tmp_path / "conversion-publish-stop"
    result = runner.run_formal_attempt(
        output,
        replicate="A",
        dependency_audit_token=token_path,
        invocation_argv=("synthetic-conversion-publish-test",),
    )
    assert result["status"] == "STOP"
    summary = json.loads((output / "summary.json").read_text())
    assert summary["stop_code"] == "stage_evidence_conversion_error"
    assert summary["terminal_coordinate"] == {
        "transport_substeps": 47,
        "layer": 1,
        "source_step_index": 4,
    ...
    assert summary["row_counts"]["source_events"] == 1
    assert summary["row_counts"]["transport_stages"] == 0
    reverified = runner.verify_artifact(output)
    assert reverified["status"] == "STOP"
    assert reverified["semantic_result_sha256"] == result["semantic_result_sha256"]


def test_commit_rejects_same_key_source_with_different_canonical_bytes() -> None:
    full = _pass_records()
    sink = runner.ArtifactSink()
    sink.add_source_event(full.source_events[0])
    for row in full.transport_stages[: 3 * 47]:
        _append_compact_stage(sink, dict(row))
    inconsistent = dict(full.source_events[0])
    inconsistent["kelvin_residual_m2_s"] = [
        value + 1.0e-9 for value in inconsistent["kelvin_residual_m2_s"]
    ]
    with pytest.raises(ValueError, match="precommitted source event differs"):
        sink.commit_completed_layer(
            raw_step=full.raw_steps[0],
            source_event=inconsistent,
            owner_event=full.owner_events[0],
            particle_count=full.particle_counts[0],
            raw_loads=full.raw_loads[:17],
            trajectory_array_record=full.trajectories[0],
    ...
    assert len(sink.source_events) == 1
    assert len(sink.transport_stages) == 3 * 47
    assert not sink.raw_steps


def test_completed_stage_rejects_substep_delta_time_drift() -> None:
    records = _pass_records()
    drifted = dict(records.transport_stages[0])
    drifted["substep_delta_time"] = float(
        np.nextafter(drifted["substep_delta_time"], np.inf)
    )
    hostile = replace(
        records, transport_stages=(drifted, *records.transport_stages[1:])
    )
    with pytest.raises(ValueError, match="stage substep delta time drift"):
        runner._validate_records(hostile)
