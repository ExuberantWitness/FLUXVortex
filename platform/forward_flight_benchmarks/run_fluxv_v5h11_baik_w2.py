"""No-observation runner and artifact protocol for the v5h11 Baik-W2 B3 gate.
...
SUMMARY_SCHEMA_BY_MODE: Final = {
    SYNTHETIC_EXECUTION_MODE: "fluxv-v5h11-baik-w2-synthetic-summary-v2",
    FORMAL_EXECUTION_MODE: "fluxv-v5h11-baik-w2-formal-summary-v2",
}
RUN_MANIFEST_SCHEMA_BY_MODE: Final = {
    SYNTHETIC_EXECUTION_MODE: "fluxv-v5h11-baik-w2-synthetic-run-manifest-v2",
    FORMAL_EXECUTION_MODE: "fluxv-v5h11-baik-w2-formal-run-manifest-v2",
}
TRAJECTORY_SCHEMA_BY_MODE: Final = {
    SYNTHETIC_EXECUTION_MODE: "fluxv-v5h11-baik-w2-synthetic-trajectories-v2",
    FORMAL_EXECUTION_MODE: "fluxv-v5h11-baik-w2-formal-trajectories-v2",
}
CONVERGENCE_SCHEMA_BY_MODE: Final = {
    SYNTHETIC_EXECUTION_MODE: "fluxv-v5h11-baik-w2-synthetic-convergence-v2",
    FORMAL_EXECUTION_MODE: "fluxv-v5h11-baik-w2-formal-convergence-v2",
}
DEPENDENCY_MANIFEST_SCHEMA_ID: Final = "fluxv-v5h11-dependency-manifest-v2"
DEPENDENCY_AUDIT_TOKEN_SCHEMA_ID: Final = "fluxv-v5h11-dependency-audit-token-v1"
...
FORMAL_EXECUTOR_FACTORY_NAME: Final = "build_fluxv_v5h11_w2_executor"
FORMAL_AUDIT_SCOPE: Final = "v5h11_b3_no_gt_formal_execution"
CASE_ID: Final = "W2"
...
CANONICAL_PROJECT_LEAF_RELATIVE_PATH: Final = {
    "prereg_freeze": (
        ...
        "fluxv_v5_nextgen_20260814/refine-logs/v5h11_ir_wrk3/PREREG_FREEZE.json"
    ),
    "runner": "platform/forward_flight_benchmarks/run_fluxv_v5h11_baik_w2.py",
    "runner_test": "platform/tests/test_run_fluxv_v5h11_baik_w2.py",
    "formal_executor_module": (
        "platform/forward_flight_benchmarks/fluxv_v5h11_baik_w2_executor.py"
    ),
    "formal_executor_test": "platform/tests/test_fluxv_v5h11_baik_w2_executor.py",
    "probe_source_v5h7": (
    ...
    "rvpm_ir_wrk3_stream": "src/fluxvortex/rvpm_ir_wrk3_stream.py",
...
class ArtifactSink:
    ...
    def commit_completed_layer(
        ...
        *,
        ...
        if len(current_sources) not in (layer_index, layer_index + 1):
            raise ValueError("source prefix is not synchronized with layer commits")
        source_bytes = _canonical_json_record_bytes(source_event)
        source_already_committed = len(current_sources) == layer_index + 1
        if (
            source_already_committed
            and self._source_events[layer_index] != source_bytes
        ):
        ...
        candidate = ArtifactRecords(
            ...
            source_events=(
                ...
                else (*self.source_events, dict(source_event))
            ),
        ...
        if not source_already_committed:
            self._source_events.append(source_bytes)
...
FORMAL_EXECUTOR_API: Final = FormalExecutorAPI(
    schema_id="fluxv-v5h11-baik-w2-formal-executor-api-v1",
...
def _load_formal_executor(
    ...
    module_name = (
        "fluxv_v5h11_w2_audited_executor_" + str(reverified["audit_token_sha256"])[:16]
    )
...
def _id_sequence(value: object) -> dict[str, object]:
    ...
    items = list(value)
    digest = _payload_sha256("fluxv-v5h11-particle-id-sequence-v1", items)
...
def _decode_id_sequence(value: object) -> tuple[str, ...]:
    ...
    expected = _payload_sha256("fluxv-v5h11-particle-id-sequence-v1", items)
...
def _validate_owner_semantics(
    ...
    for row in owner_rows:
        ...
        if row["schema_id"] != "fluxv-v5h11-baik-w2-owner-event-v1":
...
def _validate_trajectory_metadata(row: Mapping[str, object]) -> None:
    ...
    if type(prehistory) is not list or metadata[
        "source_prehistory_manifest_sha256"
    ] != _payload_sha256("fluxv-v5h11-source-prehistory-manifests-v1", prehistory):
    ...
    if metadata["particle_id_sequence_sha256"] != _payload_sha256(
        "fluxv-v5h11-particle-id-sequence-v1", list(row["particle_ids"])
    ):
    ...
    *,
    ...
    if status == "PASS":
        if keys != EXPECTED_STAGE_KEYS or any(
            ...
            raise ValueError("PASS transport stages are not the exact 2016-row matrix")
    elif status == "PREFIX":
    ...
    for row in rows:
        level, layer, _, stage = _stage_key(row)
        ...
        if row["status"] == "completed":
            if row["substep_delta_time"] != DELTA_TIME_S / level:
...
def _semantic_root(file_sha256: Mapping[str, str]) -> str:
    ...
    return _payload_sha256(
        "fluxv-v5h11-baik-w2-semantic-result-v1",
...
def verify_artifact(directory: Path) -> dict[str, object]:
    ...
    dependency_digest = _payload_sha256(
        "fluxv-v5h11-dependency-evidence-v1", expected_dependency
    )
    if (
...
def _publish_artifact_core(
    ...
    *,
    ...
    dependency_audit_sha256 = _payload_sha256(
        "fluxv-v5h11-dependency-evidence-v1", dependency_evidence
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        return 2
    print(
        f"{verification['status']} v5h11 no-GT inner artifact; "
