"""Observation-free Baik-W2 coupling for the v5h11 IR-WRK3 gate.
...
v5h11 layer result unless the transport handoff closes completely.
"""
...
COUPLING_INTERFACE_ID: Final = "fluxv-v5h11-baik-w2-ir-wrk3-coupling-v1"
CASE_ID: Final = "W2"
...
@dataclass(frozen=True, slots=True)
class V5H11GlobalRowCommitRequest:
...
class V5H11GlobalRowCommitter(Protocol):
    def __call__(self, request: V5H11GlobalRowCommitRequest, /) -> object:
        ...


@dataclass(frozen=True, slots=True)
class V5H11RowCommitEnvelope:
    """Exact row-owner commit plus source-event Kelvin evidence."""

    commit_result: RowCommitResult
    source_kelvin_evidence: V5H11SourceKelvinEvidence


@dataclass(frozen=True, slots=True)
class V5H11SourceKelvinEvidence:
...
def _kelvin_payload(value: V5H11SourceKelvinEvidence) -> dict[str, object]:
    return {
...
def make_v5h11_source_kelvin_evidence(
    *,
    source_step_index: int,
    row_owner_sha256: str,
    source_event_sha256: str,
    ...
    draft = V5H11SourceKelvinEvidence(
    ...
    return replace(
        ...
        evidence_sha256=_hash_payload(
            "fluxv-v5h11-source-kelvin-v1", _kelvin_payload(draft)
        ),
    )


def validate_v5h11_source_kelvin_evidence(
    value: V5H11SourceKelvinEvidence,
) -> V5H11SourceKelvinEvidence:
    _assert_execution_bindings()
    if type(value) is not V5H11SourceKelvinEvidence:
    ...
    if (
        ...
        != _hash_payload("fluxv-v5h11-source-kelvin-v1", _kelvin_payload(value))
    ):
...
def _load_ledger_payload(ledger: V5H11LayerLoadLedger) -> dict[str, object]:
    return {
...
def make_v5h11_layer_load_ledger(
    *,
    ptera_step_index: int,
    parent_state_sha256: str,
    reference_chord_m: float,
    ...
    return replace(
        ...
        ledger_sha256=_hash_payload(
            "fluxv-v5h11-load-ledger-v1", _load_ledger_payload(draft)
        ),
    )


def validate_v5h11_layer_load_ledger(
    ledger: V5H11LayerLoadLedger,
) -> V5H11LayerLoadLedger:
    _assert_execution_bindings()
    if type(ledger) is not V5H11LayerLoadLedger:
    ...
    expected = _hash_payload("fluxv-v5h11-load-ledger-v1", _load_ledger_payload(ledger))
...
class V5H11LayerResult:
    ...
    load_ledger: V5H11LayerLoadLedger
    load_ledger_sha256: str
    source_kelvin_evidence: V5H11SourceKelvinEvidence
    source_kelvin_evidence_sha256: str
    stability_envelope: V5H11StabilityEnvelope
    support_envelope: V5H11SupportEnvelope
    counters: V5H11LayerCounters
    parent_token: str
...
def _stability_payload(value: V5H11StabilityEnvelope) -> dict[str, object]:
    return {
...
def _support_payload(value: V5H11SupportEnvelope) -> dict[str, object]:
    return {
...
def _counter_tuple(value: V5H11LayerCounters) -> tuple[int, ...]:
    return tuple(getattr(value, field.name) for field in fields(value))


def _layer_payload(result: V5H11LayerResult) -> dict[str, object]:
    return {
...
class _StageFieldComposer:
    ...
    def observe(self, view: IRWRK3StreamStageView):
        ...
        h_j = self.substep_delta_time * float(
        ...
        h_u_sigma = self.substep_delta_time * float(
            np.max(
...
def _make_stability_envelope(
    ...
    return replace(
        ...
        envelope_sha256=_hash_payload(
            "fluxv-v5h11-stability-envelope-v1", _stability_payload(draft)
        ),
    )
...
def _make_support_envelope(
    ...
    return replace(
        draft,
        envelope_sha256=_hash_payload(
            "fluxv-v5h11-support-envelope-v1", _support_payload(draft)
        ),
    )
...
def _expected_layer_counters(
    ...
    stream: IRWRK3StreamResult,
    ledger: FDCallLedger,
    load: V5H11LayerLoadLedger,
    direct_call_count: int,
) -> V5H11LayerCounters:
    stage_count = 3 * substeps
    expected = V5H11LayerCounters(
    ...
    if _counter_tuple(expected) != (
        ...
        raise RuntimeError("v5h11 layer call ledger did not close exactly")
    return expected


def _make_live_layer_result_registry(
    transport_engine: Callable[..., V5H11LayerResult],
    *,
    ...
    lock = RLock()
    registry: dict[int, tuple[V5H11LayerResult, str]] = {}
    semantic_registry: dict[tuple[str, str, str], tuple[str, V5H11LayerResult]] = {}
    ...
    def result_semantic_key(result: V5H11LayerResult) -> tuple[str, str, str]:
    ...
    def verify_semantic_index() -> None:
        if len(registry) != len(semantic_registry):
            raise RuntimeError("v5h11 semantic registry cardinality drift")
        ...
        for result, result_digest in registry.values():
            ...
            if (
                ...
                raise RuntimeError("v5h11 semantic registry bijection drift")
            seen_result_digests.add(result_digest)

    def verify_seal() -> None:
        if compute_seal() != seal:
            raise RuntimeError("v5h11 live layer-result registry integrity drift")
        verify_semantic_index()

    def execute(
        owner: ReleaseRowOwner,
        *,
        config: V5H11BaikCouplingConfig,
        ptera_step_index: int,
        source_step_index: int,
        load_ledger: V5H11LayerLoadLedger,
        source_kelvin_evidence: V5H11SourceKelvinEvidence,
        ...
        validate_tree = _validate_v5h11_layer_result_tree
        ...
        with lock:
            ...
            if len(registry) + len(reservations) >= max_count:
                raise RuntimeError("v5h11 live layer-result registry cap exceeded")
            reservation = object()
        ...
        def verify_all() -> None:
            if (
                ...
                raise RuntimeError("v5h11 trusted runtime verifier drift")
            verify_runtime()
            verify_registry()
            if (
                globals().get("_validate_v5h11_layer_result_tree") is not validate_tree
        ...
        with lock:
            ...
            key = id(result)
            if key in registry:
                raise RuntimeError("v5h11 layer-result identity collision")
            if semantic_key in semantic_registry or any(
                entry[0] == result.result_sha256 for entry in semantic_registry.values()
            ):
                raise RuntimeError("duplicate v5h11 layer-result semantic issuance")
            reservations.pop(id(reservation))
    ...
    def attest(result: V5H11LayerResult) -> None:
        with lock:
            verify_seal()
    ...
    def assert_snapshot_unchanged(expected: tuple[object, ...]) -> None:
        with lock:
            ...
            if type(expected) is not tuple or len(expected) != 5:
                raise RuntimeError("invalid v5h11 registry snapshot")
            if observed[0] != expected[0] or observed[1] != expected[1]:
                raise RuntimeError("v5h11 registry changed during callback")
            ...
            if observed[4] != expected[4]:
                raise RuntimeError("v5h11 semantic registry changed during callback")
...
_EXECUTION_BINDING_NAMES: Final[tuple[str, ...]] = (
    ...
    "V5H11LayerResult",
    "V5H11SourceKelvinEvidence",
    "V5H11RowCommitEnvelope",
    "V5H11NativeBaikCouplingSolver",
    ...
    "release_row_transport_digest",
    "advance_release_row_transport_parent",
    "validate_v5h11_layer_load_ledger",
    "make_v5h11_source_kelvin_evidence",
    "validate_v5h11_source_kelvin_evidence",
    "_frozen_float64",
    ...
    "_validate_v5h11_layer_result_tree",
    "_execution_literal",
    "_assert_registry_function_bindings",
    "_NativePteraParentHash",
    "_NativePteraParentVelocity",
    "_execute_layer_result_transaction",
    "transport_v5h11_committed_layer",
    "make_fluxv_v5h11_baik_w2_solver",
)
...
def _execution_literal() -> tuple[object, ...]:
    return (
        ...
        V5H11LayerResult.__new__,
        V5H11LayerResult.__init__,
        V5H11LayerResult.__init__.__code__,
        V5H11LayerResult.__init__.__defaults__,
        V5H11LayerResult.__init__.__kwdefaults__,
        tuple(V5H11LayerResult.__slots__),
        tuple(
            (name, getattr(V5H11LayerResult, name))
            for name in V5H11LayerResult.__slots__
        ),
...
class _GuardedParentVelocity:
    ...
    def _verify(self) -> None:
        if (
            ...
            raise RuntimeError("v5h11 trusted callback verifier drift")
        self.verifier()
...
def _transport_v5h11_committed_layer_impl(
    owner: ReleaseRowOwner,
    *,
    config: V5H11BaikCouplingConfig,
    ptera_step_index: int,
    ...
    def preattest_post_advance_dependencies() -> None:
        ...
        if (
            V5H11LayerResult is not result_type
    ...
    if type(config) is not V5H11BaikCouplingConfig:
        raise TypeError("config must be an exact V5H11BaikCouplingConfig")
    ...
    load = validate_v5h11_layer_load_ledger(load_ledger)
    ptera_step = _exact_int("ptera_step_index", ptera_step_index)
    source_step = _exact_int("source_step_index", source_step_index)
    kelvin = validate_v5h11_source_kelvin_evidence(source_kelvin_evidence)
    ...
    preflight_result_sha256 = hash_payload("fluxv-v5h11-layer-result-v1", payload_plan)
    ...
    result_sha256 = hash_payload("fluxv-v5h11-layer-result-v1", payload_plan)
    result = result_type(
        **result_common,
...
(
    ...
    _transport_v5h11_committed_layer_impl,
)
...
def transport_v5h11_committed_layer(
    owner: ReleaseRowOwner,
    *,
    config: V5H11BaikCouplingConfig,
    ptera_step_index: int,
...
def _validate_v5h11_layer_result_tree(result: object) -> V5H11LayerResult:
    if type(result) is not V5H11LayerResult:
        raise ValueError("result must be an exact V5H11LayerResult")
    ...
    if result.scope == "formal":
        if result.transport_substeps not in FORMAL_TRANSPORT_SUBSTEPS:
            raise ValueError("formal layer result has an invalid N")
        expected_role = {32: "coarse", 64: "candidate", 128: "reference"}[
            result.transport_substeps
        ]
    elif result.scope == "synthetic":
        if not 1 <= result.transport_substeps <= MAX_TEST_SUBSTEPS:
            raise ValueError("synthetic layer result has an invalid N")
        expected_role = "synthetic"
    elif result.scope == "diagnostic_smoke":
        if result.transport_substeps != FORMAL_TRANSPORT_SUBSTEPS[0]:
            raise ValueError("diagnostic layer result must use N=32")
    ...
    load = validate_v5h11_layer_load_ledger(result.load_ledger)
    kelvin = validate_v5h11_source_kelvin_evidence(result.source_kelvin_evidence)
    ...
    if type(stability) is not V5H11StabilityEnvelope:
        raise ValueError("stability envelope has a foreign type")
    expected_stability_sha = _hash_payload(
        "fluxv-v5h11-stability-envelope-v1", _stability_payload(stability)
    )
    if (
    ...
    if type(support) is not V5H11SupportEnvelope:
        raise ValueError("support envelope has a foreign type")
    if (
        support.envelope_sha256
        != _hash_payload("fluxv-v5h11-support-envelope-v1", _support_payload(support))
    ...
    if (
        type(result.counters) is not V5H11LayerCounters
    ...
    if result.result_sha256 != _hash_payload(
        "fluxv-v5h11-layer-result-v1", _layer_payload(result)
    ):
        raise ValueError("layer result digest mismatch")
    return result


def validate_v5h11_layer_result(result: V5H11LayerResult) -> V5H11LayerResult:
    """Recompute the full compact tree and require exact issued identity."""

    _assert_execution_bindings()
    validated = _validate_v5h11_layer_result_tree(result)
...
class _BoundedV5H11SliceComplete(RuntimeError):
    pass


class V5H11NativeBaikCouplingSolver(NativePteraRVPMFeedbackSolver):
    """Native-Ptera W2 prefix with v5h11 row/IR-WRK3 transport ownership."""

    def __init__(
        self,
        unsteady_problem: Any,
        *,
        row_committer: V5H11GlobalRowCommitter,
        config: V5H11BaikCouplingConfig,
        **uvpm_kwargs: Any,
    ) -> None:
        _assert_execution_bindings()
        ...
        self.v5h11_config = config
        self._v5h11_row_committer = row_committer
        self._v5h11_previous_owner: ReleaseRowOwner | None = None
        self._v5h11_transported_parent: ParticleState | None = None
        self._v5h11_active_owner: ReleaseRowOwner | None = None
        self._v5h11_active_state: ParticleState | None = None
        self._v5h11_active_kelvin: V5H11SourceKelvinEvidence | None = None
        self._v5h11_slice_complete = False
        self.v5h11_layer_results: list[V5H11LayerResult] = []
        super().__init__(
            unsteady_problem,
        ...
        if self.num_steps != PTERA_STEPS_PER_CYCLE * PTERA_CYCLES:
            raise ValueError("v5h11 requires the frozen two-cycle W2 movement")
        if float(self.delta_time) != config.delta_time_s:
            raise ValueError("v5h11 movement delta_time disagrees with config")
        if self.num_panels != PTERA_CHORDWISE_PANELS * PTERA_SPANWISE_PANELS:
            raise ValueError("v5h11 W2 movement panel grid is not 2x8")
        if self.first_results_step != 0:
            raise ValueError("v5h11 requires native loads at every prefix step")

    @property
    def v5h11_slice_complete(self) -> bool:
        return self._v5h11_slice_complete

    def _prepare_active_row(self) -> None:
        step = int(self._current_step)
        active_steps = ACTIVE_PTERA_STEPS[: self.v5h11_config.active_layer_limit]
        if step not in active_steps:
            if self._v5h11_active_owner is not None or self._v5h3_pending is not None:
                raise RuntimeError("v5h11 active row leaked into an inactive step")
            return
        active_index = active_steps.index(step)
        source_step = ACTIVE_SOURCE_STEPS[active_index]
        previous = self._v5h11_previous_owner
        parent = self._v5h11_transported_parent
        if active_index == 0:
            if previous is not None or parent is not None:
                raise RuntimeError("first v5h11 layer unexpectedly has a parent")
        elif previous is None or parent is None:
            raise RuntimeError("continuous v5h11 layer lacks a transported parent")
        parent_sha = None if parent is None else _stream_state_sha256(parent)
        request = V5H11GlobalRowCommitRequest(
            ptera_step_index=step,
            source_step_index=source_step,
        ...
        verify()
        candidate = self._v5h11_row_committer(request)
        if (
            ...
            raise RuntimeError("v5h11 runtime verifier changed in row callback")
        verify()
        if ptera_parent_state_sha256(self) != parent_before:
            raise RuntimeError("row callback mutated native Ptera state")
        if type(candidate) is not V5H11RowCommitEnvelope:
            raise TypeError("row_committer must return V5H11RowCommitEnvelope")
        ...
        if (
            ...
            or owner.state.positions.shape[0] > self.v5h11_config.particle_cap
        ):
        ...
        kelvin = validate_v5h11_source_kelvin_evidence(candidate.source_kelvin_evidence)
        if (
            kelvin.source_step_index != source_step
            or kelvin.row_owner_sha256 != owner.owner_sha256
        ):
            raise RuntimeError("row source Kelvin evidence is not bound to the commit")
        self._v5h11_active_owner = owner
        self._v5h11_active_state = state
        self._v5h11_active_kelvin = kelvin
    ...
    def _stage_feedback(self) -> None:
        ...
        if step not in active_steps:
            return
        owner = self._v5h11_active_owner
        state = self._v5h11_active_state
        if owner is None or state is None or self._v5h11_active_kelvin is None:
            raise RuntimeError("native feedback lacks a prevalidated v5h11 row")
        ...
        if self.current_operating_point.surfaceReflect_T_act_GP1_CgP1 is not None:
            raise ValueError("v5h11 does not support Ptera image surfaces")
        active_index = active_steps.index(step)
        source_step = ACTIVE_SOURCE_STEPS[active_index]
        ...
        if not np.all(np.isfinite(combined)):
            raise FloatingPointError("v5h11 combined native Ptera RHS is non-finite")
    ...
    def _native_load_ledger(
        ...
        if len(airplanes) != 1:
            raise RuntimeError("v5h11 W2 slice requires exactly one airplane")
        ...
        ledger = make_v5h11_layer_load_ledger(
            ...
            no_penetration_atol=self.v5h11_config.no_penetration_atol,
    ...
    def _populate_next_airplanes_wake(self) -> None:
        active = self._v5h11_active_owner
        if active is None:
        ...
        parent_token = f"v5h11:ptera-step:{int(self._current_step)}:parent:{parent_sha}"
        ...
        active_index = len(self.v5h11_layer_results)
        kelvin = self._v5h11_active_kelvin
        if kelvin is None:
            raise RuntimeError("active native layer lost source Kelvin evidence")
        result = transport_v5h11_committed_layer(
            active,
            config=self.v5h11_config,
            ptera_step_index=int(self._current_step),
        ...
        if validate_v5h11_layer_result(result) is not result:
            raise RuntimeError("native layer validator changed issued identity")
        self.v5h11_layer_results.append(result)
        self._v5h11_previous_owner = result.advanced_owner
        self._v5h11_transported_parent = result.stream_result.final_state
        self._v5h11_active_owner = None
        self._v5h11_active_state = None
        self._v5h11_active_kelvin = None
        if len(self.v5h11_layer_results) == self.v5h11_config.active_layer_limit:
            self._v5h11_slice_complete = True
            raise _BoundedV5H11SliceComplete
    ...
    def run(
        ...
        if calculate_streamlines:
            raise ValueError("bounded v5h11 slice forbids streamline postprocessing")
        if self._v5h11_slice_complete:
            raise RuntimeError("bounded v5h11 slice cannot run twice")
        try:
            super().run(
        ...
        except _BoundedV5H11SliceComplete:
            if len(self.v5h11_layer_results) != self.v5h11_config.active_layer_limit:
                raise RuntimeError("v5h11 bounded stop occurred at the wrong layer")
            return
        except Exception as error:
            self._poison(error)
            raise
        raise RuntimeError("full Ptera run escaped the bounded v5h11 stop")


def make_fluxv_v5h11_baik_w2_solver(
    unsteady_problem: Any,
    *,
    row_committer: V5H11GlobalRowCommitter,
    config: V5H11BaikCouplingConfig,
    **uvpm_kwargs: Any,
    ...
    """Construct the isolated bounded native-Ptera v5h11 W2 solver."""

    _assert_execution_bindings()
    return V5H11NativeBaikCouplingSolver(
        unsteady_problem,
...
def _assert_execution_bindings(
    ...
    if globals().get("_FROZEN_EXECUTION_BINDINGS") is not frozen_bindings:
        raise RuntimeError("v5h11 execution binding registry drift")
    if globals().get("_FROZEN_EXECUTION_LITERAL") is not frozen_literal:
        raise RuntimeError("v5h11 execution literal registry drift")
    if globals().get("_EXECUTION_BINDING_NAMES") is not _EXECUTION_BINDING_NAMES:
        raise RuntimeError("v5h11 execution binding-name registry drift")
    for name, frozen in frozen_bindings:
        if globals().get(name) is not frozen:
            raise RuntimeError(f"v5h11 execution binding drift: {name}")
    if _execution_literal() != frozen_literal:
        raise RuntimeError("v5h11 execution constant drift")
    _assert_registry_function_bindings()


del _make_live_layer_result_registry
del _freeze_registry_function
del _transport_v5h11_committed_layer_impl
...
__all__ = (
    ...
    "OBSERVATION_ACCESS",
    "V5H11BaikCouplingConfig",
    "V5H11GlobalRowCommitRequest",
    "V5H11GlobalRowCommitter",
    "V5H11LayerCounters",
    "V5H11LayerLoadLedger",
    "V5H11LayerResult",
    "V5H11SourceKelvinEvidence",
    ...
    "V5H11StabilityEnvelope",
    "V5H11SupportEnvelope",
    "make_v5h11_layer_load_ledger",
    "make_v5h11_source_kelvin_evidence",
    "make_fluxv_v5h11_baik_w2_solver",
    "transport_v5h11_committed_layer",
    "validate_v5h11_layer_load_ledger",
    "validate_v5h11_source_kelvin_evidence",
    "validate_v5h11_layer_result",
)
