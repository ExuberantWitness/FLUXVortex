"""Audited no-GT executor for the V5H13 execution-only repair.

The runner is intentionally not imported here.  It injects the exact sink and
STOP class identities through ``build_fluxv_v5h13_w2_executor`` only after its
...
EXECUTOR_API_SCHEMA_ID: Final = "fluxv-v5h13-baik-w2-formal-executor-api-v1"
DISPOSABLE_SMOKE_SUMMARY_SCHEMA_ID: Final = (
    "fluxv-v5h13-baik-w2-disposable-n32-layer1-summary-v1"
)
...
def graded_substep_delta_time(level: int, substep: int) -> float:
    """The frozen V5H13 birth-window dt of one sub-step slot."""
...
STAGE_EVIDENCE_SCHEMA: Final = "v5h13-stage-fd-stability-v1"
...
SOURCE_PREHISTORY_MANIFEST_DOMAIN: Final = "fluxv-v5h13-source-prehistory-manifests-v1"
...
MODULE_LEAVES: Final = MappingProxyType(
    {
        ...
        "rvpm_ir_wrk3_v5h13_stream": "fluxvortex.rvpm_ir_wrk3_v5h13_stream",
        ...
        "v5h13_baik_coupling": ("forward_flight_benchmarks.fluxv_v5h13_baik_coupling"),
...
def _load_verified_runtime(contract: ExecutorAPIContract) -> RuntimeModules:
    ...
    return RuntimeModules(
        coupling=imported["v5h13_baik_coupling"],
        ...
        stream=imported["rvpm_ir_wrk3_v5h13_stream"],
...
class LayerCommitCapture:
    ...
    def source_cell_manifests(self) -> list[object]:
        ...
        if (
            ...
            or _payload_digest("fluxv-v5h13-source-cell-manifests-v1", value)
...
def aggregate_source_events(
    ...
    *,
    ...
    manifest_sha = _payload_digest("fluxv-v5h13-source-cell-manifests-v1", manifests)
    ...
    ledger_sha = _payload_digest("fluxv-v5h13-source-kelvin-ledgers-v1", ledgers)
    row: dict[str, object] = {
        "transport_substeps": level,
        ...
        "event_sha256": _payload_digest(
            "fluxv-v5h13-source-event-aggregate-v1", producer_hashes
        ),
        "parent_event_sha256": _payload_digest(
            "fluxv-v5h13-source-parent-aggregate-v1", parent_hashes
        ),
    }
    ...
    *,
    ...
    metadata = {
        ...
        "particle_id_sequence_sha256": _payload_digest(
            "fluxv-v5h13-particle-id-sequence-v1", list(capture.particle_ids)
        ),
...
class BaikW2RowCommitter:
    ...
    def __init__(
        ...
        *,
        ...
        self.events = events
        self.level = level
        self.owner_id = f"fluxv-v5h13-baik-w2-formal-owner-N{level}"
    ...
    def __call__(self, request: object) -> object:
        ...
        if type(request) is not coupling.V5H13GlobalRowCommitRequest:
            raise TypeError("row request must use the exact v5h13 schema")
        ...
        kelvin = coupling.make_v5h13_source_kelvin_evidence(
            source_step_index=request.source_step_index,
        ...
        return coupling.V5H13RowCommitEnvelope(
            commit_result=result,
            source_kelvin_evidence=kelvin,
        )
...
def _make_disposable_layer1_smoke_summary(
    *,
    ...
    if runtime.coupling.validate_v5h13_layer_result(result) is not result:
        raise ExecutorContractError(
...
def _emit_stream_stop(
    *,
    ...
    if completed:
        expected_prefix = f"v5h13:ptera-step:{capture.ptera_step_index}:parent:"
...
def _run_disposable_n32_layer1_smoke(
    ...
    config = runtime.coupling.V5H13BaikCouplingConfig(
    ...
    solver = runtime.coupling.make_fluxv_v5h13_baik_w2_solver(
        problem,
    ...
    results = tuple(getattr(solver, "v5h13_layer_results", ()))
...
def _run_formal_level(
    ...
    try:
        ...
        completed_results = tuple(getattr(solver, "v5h13_layer_results", ()))
        if (
...
def build_fluxv_v5h13_w2_executor(api: object) -> FormalCouplingExecutor:
