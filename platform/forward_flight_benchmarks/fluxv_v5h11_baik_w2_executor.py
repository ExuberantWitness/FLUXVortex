"""Audited no-GT executor for the v5h11 Baik-W2 formal B3 matrix.

The runner is intentionally not imported here.  It injects the exact sink and
STOP class identities through ``build_fluxv_v5h11_w2_executor`` only after its
...
EXECUTOR_API_SCHEMA_ID: Final = "fluxv-v5h11-baik-w2-formal-executor-api-v1"
DISPOSABLE_SMOKE_SUMMARY_SCHEMA_ID: Final = (
    "fluxv-v5h11-baik-w2-disposable-n32-layer1-summary-v1"
)
...
STAGE_EVIDENCE_FIELDS: Final = frozenset(
    {
        "fd_physical_evaluation_sha256",
        "fd_tracer_evaluation_sha256",
        "h_convective_over_sigma",
...
def parse_stage_evidence(record: object) -> dict[str, object]:
    ...
    if set(decoded) != STAGE_EVIDENCE_FIELDS:
    ...
    normalized = _hex_float(
        "invariant_residual_over_slog_max",
        decoded["invariant_residual_over_slog_max"],
    )
    ...
    compact_value = getattr(record, "invariant_residual_over_slog_max", None)
    if type(compact_value) is not float or compact_value != normalized:
        raise ExecutorContractError(
            "stream compact normalized-invariant field differs from evidence"
        )
...
def emit_completed_layer(
    *,
    ...
    """Append 3N stages, then atomically commit the one completed layer bundle."""
    ...
    if not callable(add_stage) or not callable(commit_layer):
    ...
    for index, record in enumerate(stages):
        ...
        row, compact = completed_stage_row(record, result=result, layer=capture.layer)
    ...
    commit_layer(
        ...
        source_event=capture.source_row(),
...
def build_fluxv_v5h11_w2_executor(api: object) -> FormalCouplingExecutor:
