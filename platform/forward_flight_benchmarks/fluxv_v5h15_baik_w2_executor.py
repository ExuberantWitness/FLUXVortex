"""Audited no-GT executor for the V5H15 execution-only repair.

The runner is intentionally not imported here.  It injects the exact sink and
STOP class identities through ``build_fluxv_v5h15_w2_executor`` only after its
...
EXECUTOR_API_SCHEMA_ID: Final = "fluxv-v5h15-baik-w2-formal-executor-api-v1"
DISPOSABLE_SMOKE_SUMMARY_SCHEMA_ID: Final = (
    "fluxv-v5h15-baik-w2-disposable-n32-layer1-summary-v1"
)
...
def nominal_transport_substeps(level: int) -> int:
    """Map a graded effective level back to its nominal sub-step count."""

    return level - BIRTH_WINDOW_K * BIRTH_WINDOW_R + BIRTH_WINDOW_K


def graded_substep_delta_time(level: int, substep: int) -> float:
    """The frozen V5H15 birth-window dt of one sub-step slot."""

    nominal = nominal_transport_substeps(level)
...
ROW_SMOOTHING_RADIUS_M: Final = 0.00266  # 0.00152 * V5H15_SIGMA_KAPPA (1.75), frozen
...
STAGE_EVIDENCE_SCHEMA: Final = "v5h15-stage-fd-stability-v1"
...
SOURCE_PREHISTORY_MANIFEST_DOMAIN: Final = "fluxv-v5h15-source-prehistory-manifests-v1"
...
MODULE_LEAVES: Final = MappingProxyType(
    {
        ...
        "rvpm_ir_wrk3_v5h15_stream": "fluxvortex.rvpm_ir_wrk3_v5h15_stream",
        ...
        "v5h15_baik_coupling": ("forward_flight_benchmarks.fluxv_v5h15_baik_coupling"),
...
def _load_verified_runtime(contract: ExecutorAPIContract) -> RuntimeModules:
    ...
    return RuntimeModules(
        coupling=imported["v5h15_baik_coupling"],
        ...
        stream=imported["rvpm_ir_wrk3_v5h15_stream"],
...
def completed_stage_row(
    ...
    *,
    ...
    row = {
        ...
        "stage": stage,
        "status": "completed",
        "substep_delta_time": getattr(record, "substep_delta_time"),
    ...
    row.update(
        {
            ...
            "stage": stage,
            "status": "failed",
            "substep_delta_time": graded_substep_delta_time(level, substep),
            "rk_a": RK_A[stage - 1],
            "rk_b": RK_B[stage - 1],
...
class LayerCommitCapture:
    ...
    def source_cell_manifests(self) -> list[object]:
        ...
        if (
            ...
            or _payload_digest("fluxv-v5h15-source-cell-manifests-v1", value)
...
def aggregate_source_events(
    ...
    *,
    ...
    manifest_sha = _payload_digest("fluxv-v5h15-source-cell-manifests-v1", manifests)
    ...
    ledger_sha = _payload_digest("fluxv-v5h15-source-kelvin-ledgers-v1", ledgers)
    row: dict[str, object] = {
        "transport_substeps": level,
        ...
        "event_sha256": _payload_digest(
            "fluxv-v5h15-source-event-aggregate-v1", producer_hashes
        ),
        "parent_event_sha256": _payload_digest(
            "fluxv-v5h15-source-parent-aggregate-v1", parent_hashes
        ),
    }
    ...
    *,
    ...
    metadata = {
        ...
        "particle_id_sequence_sha256": _payload_digest(
            "fluxv-v5h15-particle-id-sequence-v1", list(capture.particle_ids)
        ),
        "material_tracer_id_sequence_sha256": _payload_digest(
            "fluxv-v5h15-material-id-sequence-v1",
...
class BaikW2RowCommitter:
    ...
    def __init__(
        ...
        *,
        ...
        self.events = events
        self.level = level
        self.owner_id = f"fluxv-v5h15-baik-w2-formal-owner-N{level}"
    ...
    def __call__(self, request: object) -> object:
        ...
        if type(request) is not coupling.V5H15GlobalRowCommitRequest:
            raise TypeError("row request must use the exact v5h15 schema")
        ...
        kelvin = coupling.make_v5h15_source_kelvin_evidence(
            source_step_index=request.source_step_index,
        ...
        return coupling.V5H15RowCommitEnvelope(
            commit_result=result,
            source_kelvin_evidence=kelvin,
        )
...
def _make_disposable_layer1_smoke_summary(
    *,
    ...
    if runtime.coupling.validate_v5h15_layer_result(result) is not result:
        raise ExecutorContractError(
    ...
    if type(stages) is not tuple or len(stages) != expected_stages:
        raise ExecutorContractError(
            "disposable layer lacks its exact graded compact stage records"
        )
...
class FormalCouplingExecutor:
    ...
    def run_formal_matrix(self, *, levels: tuple[int, int, int], sink: object) -> None:
        if type(levels) is not tuple or levels != FORMAL_LEVELS:
            raise ExecutorContractError(
                "formal executor requires the exact graded level set"
            )
...
def _emit_stream_stop(
    *,
    ...
    if completed:
        expected_prefix = f"v5h15:ptera-step:{capture.ptera_step_index}:parent:"
...
def _run_disposable_n32_layer1_smoke(
    ...
    config = runtime.coupling.V5H15BaikCouplingConfig(
        transport_substeps=nominal_transport_substeps(DISPOSABLE_SMOKE_LEVEL),
        formal_matrix=False,
        test_mode=False,
    ...
    solver = runtime.coupling.make_fluxv_v5h15_baik_w2_solver(
        problem,
    ...
    results = tuple(getattr(solver, "v5h15_layer_results", ()))
...
def _run_formal_level(
    ...
    if level not in FORMAL_LEVELS:
        raise ExecutorContractError("formal level is outside the graded set")
    events = build_w2_source_events(runtime)
    movement = build_w2_movement(runtime.pterasoftware)
    problem = runtime.pterasoftware.problems.UnsteadyProblem(
        movement=movement,
    ...
    config = runtime.coupling.V5H15BaikCouplingConfig(
        transport_substeps=nominal_transport_substeps(level),
        formal_matrix=True,
        test_mode=False,
        particle_cap=ROW_PARTICLE_CAP,
        birth_window_refinement=True,
    ...
    try:
        ...
        completed_results = tuple(getattr(solver, "v5h15_layer_results", ()))
        if (
...
def build_fluxv_v5h15_w2_executor(api: object) -> FormalCouplingExecutor:
