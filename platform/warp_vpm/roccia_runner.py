"""Baik W1-W4 runner with Roccia LESP-clipped LEV."""
...
from warp_vpm.roccia_lev import RocciaLEVSolver
...
executor = load("rex", str(repo / "platform/forward_flight_benchmarks/fluxv_v5h15_baik_w2_executor.py"))
...
print("Runtime loaded for Roccia LEV", flush=True)


class BaikRocciaSolver(RocciaLEVSolver, coupling.V5H15NativeBaikCouplingSolver):
    def __init__(self, problem, U, c, period, lesp_crit=0.11, **kw):
        super().__init__(problem, **kw)
        self._init_roccia_lev(U, c, period, lesp_crit=lesp_crit)


def run_case(case_id, lesp_crit=0.11):
...
if __name__ == "__main__":
    ...
    Path("/tmp/v5h15-paper/roccia_lev_results.json").write_text(
