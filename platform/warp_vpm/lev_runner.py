"""Baik W1-W4 runner with LDVM-driven LEV + bidirectional coupling."""
...
from warp_vpm.lev_hybrid import LEVHybridSolver
...
executor = load("lev_ex", str(repo / "platform/forward_flight_benchmarks/fluxv_v5h15_baik_w2_executor.py"))
...
class BaikLEVSolver(LEVHybridSolver, coupling.V5H15NativeBaikCouplingSolver):
    def __init__(self, problem, U, c, period, **kw):
        super().__init__(problem, **kw)
        self._init_lev_hybrid(runtime, U, c, period)
...
if __name__ == "__main__":
    ...
    print(f"\n=== LDVM LEV Hybrid Summary ===")
    print(f"CL macro RMSE: {np.mean(cl):.3f} (prev LEV: 0.792, bare: 0.793, SOTA: 0.594)")
    print(f"CD macro RMSE: {np.mean(cd):.3f} (prev LEV: 0.459, bare: 0.420, SOTA: 0.346)")
    Path("/tmp/v5h15-paper/ldvm_lev_results.json").write_text(
