...
CACHES = {
    ...
    'H2': json.load(open(os.path.join(OD, 'cache_nc4_cyc3_H2.json'))),
}
COL = {'exp': 'k', 'K0': '#1b9e77', 'H4': '#d95f02', 'H2': '#7570b3'}
LW  = {'exp': 2.4, 'K0': 1.4, 'H4': 1.4, 'H2': 1.0}
...
def draw(ax, keys, title, xlabel):
    for i, k in enumerate(keys):
        ...
        for c in ('K0', 'H4'):
            ...
            ax.plot(x, p, '--' if c == 'K0' else '-.', color=COL[c], lw=LW[c], alpha=0.85,
                    label=f'{c}' if shade else None)
    ax.set_title(title, fontsize=11); ax.set_xlabel(xlabel); ax.set_ylabel('力 (N)'); ax.grid(alpha=0.3); ax.legend(fontsize=7, ncol=3)
...
fig.suptitle('Fig17  实测(黑实线) vs K0(绿虚=kelvin基线) vs H4(橙点划=hirato+a0_crit0.27)  [8m/s AoA5]', fontsize=12)
...
fig.suptitle('Fig18  实测(黑) vs K0(绿虚) vs H4(橙点划)  [扭转0, AoA5]', fontsize=12)
...
fig.suptitle('Fig19  实测(黑) vs K0(绿虚) vs H4(橙点划)  [8m/s, 每攻角]', fontsize=12)
...
labels = list(groups.keys()); x = np.arange(len(labels)); w = 0.35
for idx, c in enumerate(('K0', 'H4')):
    Lm = []; Tm = []
    for g in labels:
        la = []; ta = []
        for k in groups[g]:
            exp = np.array(R[k]['exp'], float); p = pred_line(CACHES[c], k)
            if R[k]['kind'] == 'L': la.append(mae(p, exp))
            else: ta.append(mae(p, exp))
        Lm.append(np.mean(la) if la else 0); Tm.append(np.mean(ta) if ta else 0)
    ax[0].bar(x + (idx-0.5)*w, Lm, w, color=COL[c], label=c); ax[1].bar(x + (idx-0.5)*w, Tm, w, color=COL[c], label=c)
...
fig.suptitle('按图分组 MAE:K0 vs H4', fontsize=12); fig.tight_layout()
