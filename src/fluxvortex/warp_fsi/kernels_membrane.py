...
def _branch_forces(e1: DTYPE, e2: DTYPE,
    ...
    if nf2 > DTYPE(0.0):                         # taut
        n1r = nf1; n2r = nf2
    elif e1 > e1sl:                              # wrinkled (uniaxial)
        n1r = N0 * (DTYPE(1.0) - nu) + hE * e1
        n2r = DTYPE(0.0)
...
def _membrane_W(E: MAT22, N0: DTYPE, hb: DTYPE, hE: DTYPE,
                nu: DTYPE, eta: DTYPE) -> DTYPE:
    """Relaxed energy per unit reference area, eta-blended."""
    ...
    if nf2 <= DTYPE(0.0):
        ...
        if e1 > e1sl:                            # wrinkled
            Wr = N0 * (DTYPE(1.0) - nu) * e1 + DTYPE(0.5) * hE * e1 * e1 + Wwr_const
        else:                                    # slack
            Wr = N0 * (DTYPE(1.0) - nu) * e1sl + DTYPE(0.5) * hE * e1sl * e1sl + Wwr_const
    return (DTYPE(1.0) - eta) * Wr + eta * Wf


@wp.func
def _membrane_N(E: MAT22, N0: DTYPE, hb: DTYPE, hE: DTYPE,
                nu: DTYPE, eta: DTYPE) -> MAT22:
    """Total membrane force tensor N = dW/dE (2x2, principal reconstruction)."""
    e1, e2, sd = _principal(E)
    n1, n2 = _branch_forces(e1, e2, N0, hb, hE, nu, eta)
    a = DTYPE(0.5) * (n1 + n2)
    if sd > DTYPE(1e-14):
...
def _membrane_elem_force(p: VEC9, dX1: VEC3, dX2: VEC3, Bm: MAT22, A0: DTYPE,
    ...
    N = _membrane_N(E, N0, hb, hE, nu, eta)
...
def membrane_energy_kernel(q: wp.array(dtype=DTYPE, ndim=2),
    ...
    wp.atomic_add(W, e, A0[el] * _membrane_W(E, N0, hb, hE, nu, eta))
...
def membrane_state(q_wp, C: MembraneConstants, device=None):
    ...
    wp.launch(membrane_state_kernel, dim=(B, C.ne),
              inputs=[q_wp, C.edofs, C.dX1, C.dX2, C.Bm] + C._params(),
              outputs=[out], device=device)
    return out


def assemble_membrane_kblocks(q_wp, C: MembraneConstants, h_fd=1e-7,
