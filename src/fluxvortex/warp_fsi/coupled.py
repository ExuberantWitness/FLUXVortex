...
@wp.kernel
def dp_lift1_flat_kernel(
    gamma: wp.array(dtype=DTYPE, ndim=2),  # (B, P) flat p=i*ns+j
    ...
    dp2: wp.array(dtype=VEC3, ndim=2),
):  # (B, P) dp_lift2 vec
    e, p = wp.tid()
    i = p / ns
    j = p - i * ns
...
@wp.kernel
def dp_times_n_kernel(
    dp: wp.array(dtype=DTYPE, ndim=2),  # (B, P)
    normals: wp.array(dtype=VEC3, ndim=2),  # (B, P)
    dpn: wp.array(dtype=DTYPE, ndim=2),
):  # (B, 3P) out
    e, p = wp.tid()
    n = normals[e, p]
    v = dp[e, p]
...
@wp.kernel
def vertex_vel_kernel(
    dq: wp.array(dtype=DTYPE, ndim=2),  # (B, ndof)
    closest: wp.array(dtype=wp.int32, ndim=1),  # (NV,)
    dtv: wp.array(dtype=VEC3, ndim=2),
):  # (B, NV) out
...
@wp.kernel
def dt_normals_kernel(
    verts: wp.array(dtype=VEC3, ndim=1),  # (NV,) frozen ref grid
    dtv: wp.array(dtype=VEC3, ndim=2),  # (B, NV) vertex vel
    ns: int,
    dtn: wp.array(dtype=VEC3, ndim=2),
):  # (B, P) out
...
@wp.kernel
def mf2_1_scalar_kernel(
    vstruct: wp.array(dtype=VEC3, ndim=2),  # (B, P)
    vinf: VEC3,
    dtn: wp.array(dtype=VEC3, ndim=2),  # (B, P)
    scal: wp.array(dtype=DTYPE, ndim=2),
):  # (B, P) out
...
@wp.kernel
def mf2_1_dpn_kernel(
    x: wp.array(dtype=DTYPE, ndim=2),  # (B, P) = AIC^{-1}·scal
...
@wp.kernel
def any_abs_gt_kernel(
    values: wp.array(dtype=DTYPE, ndim=2),
    threshold: DTYPE,
    flag: wp.array(dtype=wp.int32, ndim=1),
):
    environment, index = wp.tid()
    if wp.abs(values[environment, index]) > threshold:
        wp.atomic_max(flag, 0, 1)


def _gpu_any_abs_gt(values, threshold, device):
    flag = wp.zeros(1, dtype=wp.int32, device=device)
    wp.launch(
        any_abs_gt_kernel,
        dim=values.shape,
        inputs=[values, DTYPE(threshold)],
        outputs=[flag],
        device=device,
    )
    wp.synchronize()
...
class GpuFluidSolve:
    ...
    def __init__(self, solver, scgeom=None, device=None, wake=False, wake_max_rows=64):
    ...
    def mf2_1_force(self, dq_wp, normals_held):
        ...
        wp.launch(
            dt_normals_kernel,
            dim=(B, self.P),
            inputs=[self.verts_wp, self.dtv, self.ns],
            outputs=[self.dtn],
            device=d,
        )
        ...
        wp.launch(
            mf2_1_dpn_kernel,
            dim=(B, self.P),
            inputs=[x, normals_held, DTYPE(self.rho)],
            outputs=[self.dpn_mf],
            device=d,
        )
    ...
    def solve(self, q_wp, dq_wp):
        ...
        wp.launch(
            gamma_rhs_kernel,
            dim=(B, self.P),
            inputs=[vinf, vstruct, g.normals],
            outputs=[self.rhs],
            device=d,
        )
        if wk is not None:
            induce_velocity_batched(
                g.colloc, wk.wcorners, wk.wgamma, self.core, out_V=self.Vw, device=d
            )
            wp.launch(
                add_dot_n_kernel,
                dim=(B, self.P),
                inputs=[self.Vw, g.normals],
                outputs=[self.rhs],
                device=d,
            )
        ...
        if wk is not None:
            ...
            self._g1_nonzero = _gpu_any_abs_gt(gamma, 1.0e-15, d)
        ...
        wp.launch(
            dp_lift1_flat_kernel,
            dim=(B, self.P),
            inputs=[gamma, g.corners, V_ext, vinf, DTYPE(self.rho), self.nc, self.ns],
            outputs=[self.dp, self.dp2],
            device=d,
        )
        if wk is not None and wk.n_rows > 0:
            ...
            wp.launch(
                dp_add_scaled_kernel,
                dim=(B, self.P),
                inputs=[mf2v, DTYPE(self.rho)],
                outputs=[self.dp],
                device=d,
            )
        # dp·n -> load transfer
...
@wp.kernel
def add_dot_n_kernel(
    V: wp.array(dtype=VEC3, ndim=2),  # (B, P)
    normals: wp.array(dtype=VEC3, ndim=2),  # (B, P)
    rhs: wp.array(dtype=DTYPE, ndim=2),
):  # (B, P) in/out
...
@wp.kernel
def dp_add_scaled_kernel(
    x: wp.array(dtype=DTYPE, ndim=2),  # (B, P) Mf2_vec1
    c: DTYPE,  # rho
    dp: wp.array(dtype=DTYPE, ndim=2),
):  # (B, P) in/out
    e, i = wp.tid()
...
@wp.kernel
def negate_kernel(x: wp.array(dtype=DTYPE, ndim=2), out: wp.array(dtype=DTYPE, ndim=2)):
    e, i = wp.tid()
    out[e, i] = -x[e, i]


@wp.kernel
def lift2_dpn_kernel(
    vstruct: wp.array(dtype=VEC3, ndim=2),  # (B, P) Sc_col·dq
...
def gpu_coupled_trajectory(
    ...
    dq = wp.clone(dq0)
    pulse_dev = wp.array(
        np.broadcast_to(pulse_shape, (B, ndof)).astype(NP).copy(),
        dtype=DTYPE,
        device=device,
    )
    ...
    while step < n_steps:
        for _k in range(uvlm_ratio):
            ...
            wp.launch(
                fc_combine_kernel,
                dim=(B, ndof),
                inputs=[DTYPE(NP(s)), pulse_dev, Fbern],
                outputs=[Fc],
                device=device,
            )
