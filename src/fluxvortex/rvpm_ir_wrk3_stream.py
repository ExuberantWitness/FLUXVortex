...
def _validate_result_tree(result: object) -> IRWRK3StreamResult:
    ...
    substep_delta_time = result.delta_time / result.transport_substeps
    if not math.isfinite(substep_delta_time) or substep_delta_time <= 0.0:
        raise ValueError("result per-substep delta_time underflowed or is invalid")
...
def ir_wrk3_stream_macro(
    ...
    *,
    ...
    substep_delta_time = float(delta_time) / transport_substeps
    if not math.isfinite(substep_delta_time) or substep_delta_time <= 0.0:
