...
for bi, b in enumerate(boundaries):
    ...
    tf = b * d_t
    prev_b = b
    print(f"  boundary {b:4d} t={b*d_t:.3f}  tip={X[zdof]:+.6e}  wake_rows={wake['r2'].shape[0]//Ny_ if wake else 0}  ({tmod.time()-t0:.0f}s)", flush=True)
    np.save(f'/tmp/chain_X_b{b}.npy', X)
