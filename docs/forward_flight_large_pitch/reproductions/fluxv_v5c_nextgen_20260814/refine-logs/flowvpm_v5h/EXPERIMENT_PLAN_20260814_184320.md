...
4. source-frozen baseline 取 `lambda=2.125`；不得根据三论文表现选择。
5. 消融只比较三项：fixed-sigma cVPM、dynamic-sigma rVPM、rVPM+corrected relaxation。SFS/viscosity/FMM 暂不加入。

监测量：

- `Gamma*sigma^2` 的 reformulated invariant；
- Kelvin、总 vector moment、冲量代理和质心轨迹；
- `min_spacing/sigma`、`dt*||J||`、`dt*|U|/sigma`；
