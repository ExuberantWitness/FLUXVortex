...
|---|---|---|---|---|---|---|---|
| Q16-H0-001 | H0 | 冻结 Q16 节点序、96 DOF、积分、ANS/EAS 与设计 schema | Q16 合同 | schema/hash/exact API | MUST | TODO | 任一 Q9 或 runtime p-switch → STOP |
| Q16-H0-002 | H0 | 冻结 GPU/LEV/wake 生产边界 | Q16 FSI config | CUDA float64、LEV/wake 无关闭入口 | MUST | TODO | 发现 CPU fallback 或 disable switch → STOP |
| Q16-H1-001 | H1 | 形函数与导数恒等式 | 单 Q16 | unity/Kronecker/derivative ≤64eps | MUST | TODO | 失败不写 CUDA force kernel |
| Q16-H1-002 | H1 | 三次场和刚体运动再现 | 单 Q16 | displacement/strain/energy | MUST | TODO | 刚体内力非零 → STOP |
| Q16-H1-003 | H1 | 膜、弯曲、剪切、厚度压缩 patch | 单 Q16/2×2 Q16 | patch residual | MUST | TODO | 不得用放宽工程容差通过 |
| Q16-H1-004 | H1 | energy-force 与 tangent 一致性 | 随机畸变 Q16 | directional derivative、Jv | MUST | TODO | force ≤1e-7，Jv ≤5e-6 |
| Q16-H1-005 | H1 | 质量、质心、刚体惯量 | 单 Q16/2×2 Q16 | exact mass/moment | MUST | TODO | 设计厚度/密度需同时闭合 |
| Q16-H1-006 | H1 | CUDA 与独立 Q16 oracle 一致 | small mesh | force/mass/Jv ≤1e-10 | MUST | TODO | CPU oracle 只用于测试 |
...
| Q16-H2-006 | H2 | 结构非线性求解失败闭合 | Newton–Krylov | residual/iteration/rollback | MUST | TODO | max-iter 后正常返回 → FAIL |
| Q16-H3-001 | H3 | 任意 aero point 的 Q16 几何映射 | distorted Q16 | position/velocity/Jacobian | MUST | TODO | 不允许节点=气动角点捷径 |
| Q16-H3-002 | H3 | 功共轭载荷传递 | random q/f/δq | virtual work ≤1e-11 | MUST | TODO | 四等分传递负控必须失败 |
| Q16-H3-003 | H3 | 总力/总矩守恒 | pressure fields | normalized residual ≤1e-11 | MUST | TODO | director lever arm 必须计入 |
| Q16-H3-004 | H3 | separated LEV 强制启用 | separated maneuver | nonzero LEV release/feedback | MUST | TODO | off/zero-owner 入口首步前拒绝 |
| Q16-H3-005 | H3 | predictor 真实尾迹事务 | injected failure/retry | commit=0/1、hash、fresh parity | MUST | TODO | trial 期真实 wake 变化 → STOP |
| Q16-H3-006 | H3 | CUDA-only 运行时门 | representative FSI | host calls/device/dtype | MUST | TODO | `.numpy()`/host solve/implicit upload → STOP |
