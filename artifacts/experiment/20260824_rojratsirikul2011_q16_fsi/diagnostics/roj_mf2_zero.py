"""DIAGNOSTIC ONLY: zero the mf2 wake-history pressure term to check the
remaining physics converges to the correct steady normal force."""
import sys, math
...
def zero_mf2(self, *, structural_state, geometry, aic, gamma, external_flow,
             gamma_gradient, mf2_history):
    return orig(self, structural_state=structural_state, geometry=geometry, aic=aic,
        gamma=gamma, external_flow=external_flow, gamma_gradient=gamma_gradient,
        mf2_history=torch.zeros_like(mf2_history))
AL.Q16NativeAuthorLoadAssembler.assemble = zero_mf2
...
print(f"alpha=16 deg; paper Cn ~ 0.92-0.95")
for step in range(1, 41):
    proposal = owner.propose(solver, state, velocity)
    owner.commit(proposal)
    if step in (1,2,5,10,20,30,40,50,60):
        fz = float((proposal.load.total_force @ normal_t).item())
        d = proposal.trial_state.diagnostics[-1]
        print(f"step {step:3d}: Cn={fz/qs:+8.4f}  parts={d['particle_count']:6d} lev={d['lev_release_count']}")
