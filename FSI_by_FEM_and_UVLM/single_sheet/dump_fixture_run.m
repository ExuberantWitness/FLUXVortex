%% ========================================================================
%  dump_fixture_run.m — Phase 3b alignment fixture generator
%  Mirrors run_sim.m setup but:
%    - Yamano benchmark mesh (Nx=15, Ny=10)
%    - End_Time = 0.25 (just past t* = 0.1995 dump target)
%    - Globals TIME_FOR_DUMP / DUMP_FIXTURE_PATH / DUMP_DONE drive the dump
%      hook inside calc_fluid_force.m
%  Output: fixture_t0.1995.mat with full solver scope at the chosen time.
%% ========================================================================

clc; clear all; close all hidden;

%% Setup paths (identical to run_sim.m)
...
fprintf('=== Dumping Phase 3b alignment fixture ===\n');

%% Globals to drive dump hook in calc_fluid_force.m
global TIME_FOR_DUMP DUMP_FIXTURE_PATH DUMP_DONE
TIME_FOR_DUMP = 0;
DUMP_FIXTURE_PATH = fullfile(script_dir, 'fixture_t0.1995.mat');
DUMP_DONE = [];

%% Parameters (Yamano-matched: U*=25, M*=1, AR=1, 15x10)
param_ver = 12.0;

End_Time = 0.25;      % stop just past dump target
d_t = 2e-3;
core_num = 4;
speed_check = 0;
alpha_v = 0.5;
...
%% Yamano benchmark grid (NOT the run_sim.m quick-test 10x6!)
Nx = 15;
Ny = 10;
...
exe_ver = 12.0;
fprintf('Solver version: %.1f, mesh: %dx%d, End_Time: %.2f\n', exe_ver, Nx, Ny, End_Time);
...
%% Time loop (mirrors exe.m, with TIME_FOR_DUMP update)
tic
i_time = 1;
i_time_cnt = 1;
i_wake_time = 1;
...
while (time <= time_m(end) || ~fluid_compute_flag) && isempty(DUMP_DONE)
    time = i_time*d_t;
    TIME_FOR_DUMP = time;   %% drive dump hook inside calc_fluid_force.m
...
fprintf('\n=== Done ===\n');
if ~isempty(DUMP_DONE)
    fprintf('Fixture written: %s\n', DUMP_FIXTURE_PATH);
else
    fprintf('WARNING: dump did NOT trigger. Check End_Time/dt_wake_per_dt.\n');
end
